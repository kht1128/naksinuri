	            		...
	            		<div class="card-block bg-white p-20">
	            			<!-- 
	              			<button type="button" class="btn btn-floating btn-sm btn-warning">
	              			 ...
	              			 -->
	              			<span class="ml-15 font-weight-400">신규방문자 PV</span>
	              			<div class="content-text text-center mb-0">
	                			<span class="font-size-40 font-weight-400"><fmt:formatNumber value="${newpvvisittotCnt}" pattern="#,###" /></span>
	                			<p class="blue-grey-400 font-weight-100 m-0">
	                				<c:choose>
		                				<c:when test="${newpvvisittotCnt - newpvvisittotCntPrev > 0}">
		                					<i class="text-success icon wb-triangle-up font-size-20 text-middle"></i>&nbsp;
		                					<c:out value="${(newpvvisittotCnt - newpvvisittotCntPrev)}"/>
		                					&nbsp;뷰 증가&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<fmt:formatNumber type="percent" value="${(newpvvisittotCnt-newpvvisittotCntPrev)/newpvvisittotCntPrev}"  pattern="0.00%"/>	
		                				</c:when>
		                				<c:when test="${newpvvisittotCnt - newpvvisittotCntPrev < 0}">
		                					<i class="text-danger icon wb-triangle-down font-size-20 text-middle"></i>&nbsp;
		                					<c:out value=" ${(newpvvisittotCnt - newpvvisittotCntPrev)*-1}"/>
		                					&nbsp;뷰 감소&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<fmt:formatNumber type="percent" value="${(newpvvisittotCntPrev-newpvvisittotCnt)/newpvvisittotCntPrev}"  pattern="0.00%"/>
		                				</c:when>
		                				<c:otherwise>&nbsp;-&nbsp;뷰</c:otherwise>
		                			</c:choose>
		                		</p>
	          		...
	          		<div class="card card-shadow mb-0">
	            		<div class="card-block bg-white p-20">
	            			<!-- 
	              			...
	              			-->
	              			<span class="ml-15 font-weight-400">재방문자 PV</span>
	              			<div class="content-text text-center mb-0">
	                			<span class="font-size-40 font-weight-400"><fmt:formatNumber value="${repvvisittotCnt}" pattern="#,###" /></span>
	                			<p class="blue-grey-400 font-weight-100 m-0">
	                				<c:choose>
		                				<c:when test="${repvvisittotCnt - repvvisittotCntPrev > 0}">
		                					<i class="text-success icon wb-triangle-up font-size-20 text-middle"></i>&nbsp;
		                					<c:out value="${(repvvisittotCnt - repvvisittotCntPrev)}"/>
		                					&nbsp;뷰 증가&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<fmt:formatNumber type="percent" value="${(repvvisittotCnt-repvvisittotCntPrev)/repvvisittotCntPrev}"  pattern="0.00%"/>	
		                				</c:when>
		                				<c:when test="${revisittotCnt - revisittotCntPrev < 0}">
		                					<i class="text-danger icon wb-triangle-down font-size-20 text-middle"></i>&nbsp;
		                					<c:out value=" ${(repvvisittotCnt - repvvisittotCntPrev)*-1}"/>
		                					&nbsp;뷰 감소&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<fmt:formatNumber type="percent" value="${(repvvisittotCntPrev-repvvisittotCnt)/repvvisittotCntPrev}"  pattern="0.00%"/>
		                				</c:when>
		                				<c:otherwise>&nbsp;-&nbsp;뷰</c:otherwise>
		                			</c:choose>
	                			</p>
				...
				<div class="panel-body">
					...
					<thead>
						<tr>
							...
							<th class="text-center text-middle">페이지뷰(PV)</th>
							<th class="text-center text-middle">전일대비</th>
							<th class="text-center text-middle">신규방문자PV</th>
							<th class="text-center text-middle">재방문자PV</th>                      
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-center text-middle"><c:out value="${item.VISIT_PV_CNT}"/></td>
							<td class="text-center text-middle">
								<c:choose>
	                				<c:when test="${item.VISIT_PV_CNT - item.VISIT_PV_CNT_PREV > 0}">
	                					<i class="text-success icon wb-triangle-up font-size-20 text-middle"></i>&nbsp;<c:out value="${(item.VISIT_PV_CNT - item.VISIT_PV_CNT_PREV)}"/>
	                					<span class="d-none">&nbsp;증가</span>	
	                				</c:when>
	                				<c:when test="${item.VISIT_PV_CNT - item.VISIT_PV_CNT_PREV < 0}">
	                					<i class="text-danger icon wb-triangle-down font-size-20 text-middle"></i>&nbsp;<c:out value=" ${(item.VISIT_PV_CNT - item.VISIT_PV_CNT_PREV)*-1}"/>
	                					<span class="d-none">&nbsp;감소</span>
	                				</c:when>
	                				<c:otherwise>-</c:otherwise>
	                			</c:choose>							
							</td>
							<td class="text-center text-middle"><c:out value="${item.VISIT_N_MBR_PV_CNT}"/></td>
							<td class="text-center text-middle"><c:out value="${item.VISIT_RE_PV_CNT}"/></td>							
...
$(function(){
	...
	var seriesdataitem4 = new Array();
		labelsdata.push('x');
		seriesdataitem1.push('페이지뷰');
		seriesdataitem2.push('신규방문자PV');
		seriesdataitem3.push('재방문자PV');
		seriesdataitem4.push('방문횟수');
	for (i=0; i<visitdaytotlistjson.length; i++) {
		var item = visitdaytotlistjson[i];
		labelsdata.push(item.search_dt);
		seriesdataitem1.push(item.visit_pv_cnt);
		seriesdataitem2.push(item.visit_n_mbr_pv_cnt);
		seriesdataitem3.push(item.visit_re_pv_cnt);
		seriesdataitem4.push(item.visit_cnt);
	}
...
function fnSearch(days){
	...
	document.listForm.action = "<c:url value='/eduadm/analytics/visit/pv.do'/>";
