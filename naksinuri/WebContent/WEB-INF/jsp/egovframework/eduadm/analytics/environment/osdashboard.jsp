	            		...
	            		<div class="card-block bg-white p-20">
	            			<!-- 
	              			<button type="button" class="btn btn-floating btn-sm btn-warning">
	              			 ...
	              			 -->
	              			<span class="ml-15 font-weight-400">PC웹 점유율</span>
	              			<div class="content-text text-center mb-0">
	                			<span class="font-size-40 font-weight-400"><fmt:formatNumber type="percent" value="${visitospctotCnt/(visitospctotCnt+visitosmobiletotCnt+visitosetctotCnt)}"  pattern="0.00%"/></span>
	                			<p class="blue-grey-400 font-weight-100 m-0">
	                				<c:choose>
		                				<c:when test="${visitospctotCnt - visitospctotCntPrev > 0}">
		                					<i class="text-success icon wb-triangle-up font-size-20 text-middle"></i>&nbsp;
		                					<c:out value="${(visitospctotCnt - visitospctotCntPrev)}"/>
		                					&nbsp;건 증가<%--&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<fmt:formatNumber type="percent" value="${(visitospctotCnt-visitospctotCntPrev)/visitospctotCntPrev}"  pattern="0.00%"/> --%>	
		                				</c:when>
		                				<c:when test="${visitospctotCnt - visitospctotCntPrev < 0}">
		                					<i class="text-danger icon wb-triangle-down font-size-20 text-middle"></i>&nbsp;
		                					<c:out value=" ${(visitospctotCnt - visitospctotCntPrev)*-1}"/>
		                					&nbsp;건 감소<%--&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<fmt:formatNumber type="percent" value="${(visitospctotCntPrev-visitospctotCnt)/visitospctotCntPrev}"  pattern="0.00%"/>--%>
		                				</c:when>
		                				<c:otherwise>&nbsp;-&nbsp;건</c:otherwise>
		                			</c:choose>
	                			</p>
	              			</div>
	            		</div>
	            		...
	            		<div class="card-block bg-white p-20">
	            			<!-- 
	              			<button type="button" class="btn btn-floating btn-sm btn-warning">
	              			 ...
	              			 -->
	              			<span class="ml-15 font-weight-400">모바일 점유율</span>
	              			<div class="content-text text-center mb-0">
	                			<span class="font-size-40 font-weight-400"><fmt:formatNumber type="percent" value="${visitosmobiletotCnt/(visitospctotCnt+visitosmobiletotCnt+visitosetctotCnt)}"  pattern="0.00%"/></span>
	                			<p class="blue-grey-400 font-weight-100 m-0">
	                				<c:choose>
		                				<c:when test="${visitosmobiletotCnt - visitosmobiletotCntPrev > 0}">
		                					<i class="text-success icon wb-triangle-up font-size-20 text-middle"></i>&nbsp;
		                					<c:out value="${(visitosmobiletotCnt - visitosmobiletotCntPrev)}"/>
		                					&nbsp;건 증가<%--&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<fmt:formatNumber type="percent" value="${(visitosmobiletotCnt-visitosmobiletotCntPrev)/visitosmobiletotCntPrev}"  pattern="0.00%"/> --%>	
		                				</c:when>
		                				<c:when test="${visitosmobiletotCnt - visitosmobiletotCntPrev < 0}">
		                					<i class="text-danger icon wb-triangle-down font-size-20 text-middle"></i>&nbsp;
		                					<c:out value=" ${(visitosmobiletotCnt - visitosmobiletotCntPrev)*-1}"/>
		                					&nbsp;건 감소<%--&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<fmt:formatNumber type="percent" value="${(visitosmobiletotCntPrev-visitosmobiletotCnt)/visitosmobiletotCntPrev}"  pattern="0.00%"/> --%>
		                				</c:when>
		                				<c:otherwise>&nbsp;-&nbsp;건</c:otherwise>
		                			</c:choose>
		                		</p>
				...
				<div class="panel-body">
					...
					<thead>
						<tr>
							...
							<th class="text-center text-middle">운영체제</th>
							<th class="text-center text-middle">브라우저</th>
							<th class="text-center text-middle">방문자수</th>
							<th class="text-center text-middle">비율</th>
					  	</tr>
					</thead>
					<tbody>
						<c:forEach var="item" items="${visitosdetailalllist}" varStatus="status">
						<tr>
							<td class="text-center text-middle"><c:out value="${status.count}"/></td>
							<td class="text-left text-middle"><c:out value="${item.VISIT_FLATFORM}"/></td>
							<td class="text-left text-middle"><c:out value="${item.VISIT_BROWSER}"/></td>
							<td class="text-center text-middle"><c:out value="${item.SEARCH_CNT}"/></td>
							<td class="text-center text-middle"><fmt:formatNumber type="percent" value="${item.SEARCH_CNT/item.SEARCH_TOTCNT}"  pattern="0.00%"/></td>
						</tr>
						</c:forEach>
					</tbody>
					</table>
...
$(function(){
	...
	// 차트(그래프) 영역
	//--------------------------------------------------
	var listjson1 = JSON.parse('${visitosalllistJson}');
	var seriesdata = new Array();
	var labelsdata = new Array();
	var labelsdataitem1 = new Array();
		labelsdata.push('x');
		labelsdataitem1.push("운영체제");
	var count = 1;
	for (i=0; i<listjson1.length; i++) {
		if(count > 10) break;
		var item = listjson1[i];
		if(item.visit_input_tb!='etc') {
			labelsdata.push(item.visit_flatform);
			labelsdataitem1.push(item.search_cnt/item.search_totcnt);
			count++;
		}
	}
...
function fnSearch(days){
	...
	document.listForm.action = "<c:url value='/eduadm/analytics/environment/osdashboard.do'/>";
