...
<form:form commandName="eduBoardVO" id="listForm" name="listForm" method="post" enctype="multipart/form-data" onsubmit="return frm_submit()" 
	action="/eduadm/board/notice/modify_act.do"> 
		...
		<div class="panel">
			<div class="panel-body">
				<div class="form-group row">
					<label class="col-md-2 form-control-label">상단고정</label>
					<div class="col-md-10">
						<select class="form-control selectpicker_manual" data-style="btn-outline btn-primary" name="BD_TOP_FIX_ST">
							<option value="Y" <c:if test="${info.BD_TOP_FIX_ST eq 'Y'}">selected</c:if>>고정함</option>
		        			<option value="N" <c:if test="${info.BD_TOP_FIX_ST eq 'N'}">selected</c:if>>고정안함</option>
		      			</select>
					</div>
				</div>	
				<div class="form-group row">
				...
				<div class="form-group row">
					<label class="col-md-2 form-control-label" >첨부파일</label>
		           	<div class="col-md-10">		            	
						...
						<input type="hidden" name="returnUrl" value="/eduadm/board/notice/modify.do">
...
function listact(obj) {
	location.href="/eduadm/board/notice/list.do";
}
function frm_submit() {
