...
<form:form commandName="eduBoardVO" id="listForm" name="listForm" method="post" enctype="multipart/form-data" onsubmit="return frm_submit()" 
	action="/eduadm/board/admnotice/modify_act.do"> 
			...
			<div class="panel-body">
				<div class="form-group row">
					...
					<div class="col-md-10 pl-0">
						<select class="form-control selectpicker_manual" data-style="btn-outline btn-primary" name="BD_LOCK_ST">
							<option value="Y" <c:if test="${info.BD_LOCK_ST eq 'Y'}">selected</c:if>>노출함</option>
		        			<option value="N" <c:if test="${info.BD_LOCK_ST eq 'N'}">selected</c:if>>노출안함</option>
		      			</select>
					</div>
				</div>	
				
				<div class="form-group row">
					<label class="col-md-2 form-control-label" >노출대상</label>
					<div class="col-md-10 row">
					<c:set var="BD_TRGT_CDs" value="${fn:split(info.BD_TRGT_CD,',')}"/>

					<c:forEach var="item_category" items="${list_position_cd}">
						<c:set var="isCheckboxChecked" value=""/>
						<c:forEach var="BD_TRGT_CD" items="${BD_TRGT_CDs}">
							<c:if test="${fn:trim(BD_TRGT_CD) eq item_category.CD_ID}">
								<c:set var="isCheckboxChecked" value="checked"/>
							</c:if>
						</c:forEach>
					
						<div class="checkbox-custom mr-10">
							<input type="checkbox" id="BD_TRGT_CD_${item_category.CD_ID}" name="BD_TRGT_CD" value="${item_category.CD_ID}" ${isCheckboxChecked}>
							<label for="BD_TRGT_CD_${item_category.CD_ID}">${item_category.CD_NM}</label>
						</div>
				...
				<div class="form-group row">
					<label class="col-md-2 form-control-label" >첨부파일</label>
		           	<div class="col-md-10 pl-0">		            	
						...
						<input type="hidden" name="returnUrl" value="/eduadm/board/admnotice/modify.do">
...
function listact(obj) {
	location.href="/eduadm/board/admnotice/list.do";
}
function frm_submit() {
