	  				...
	  				<div class="input-group col-lg-12 p-0">
						...
						<div class="col-lg-4 p-0 pl-10 text-right">
							<div class="btn-group w-p100">
								<input type="text" class="form-control input-auto-enter-key" name="searchKeyword" placeholder="검색어를 입력하세요." autocomplete="off" value="${searchKeyword}" data-toggle="tooltip" data-original-title="로그내용,비고(사유) 등을 입력해주세요.">
		            	...
		            	<c:forEach var="item" varStatus="status" items="${list}">
		            		...
		            		<tr class="<c:if test="${status.index%2 eq 0}">table-active</c:if>">
		            			...
		            			<td class="text-middle text-center">
		            				<c:choose>
		            					<c:when test="${not empty item.MBR_ID }">${item.MBR_NCNM }<br>(${item.MBR_NM } , ${item.MBR_ID })</c:when>
		            					<c:otherwise>비로그인</c:otherwise>
		            				</c:choose> 
		            				<br><span class="grey-400">${item.MBR_IP }</span></td>
		            			</td>
