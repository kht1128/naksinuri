...
<div id="fishjobList" class="content respon2">
	<section id="webzineList" class="list_box">
		...
		<div class="tabArea tab3 mbNone"> <!-- 탭이 3개면 class="tabArea tab3" 4개면 뭐겠냐 -->
			<ul class="floats">
				<li class="on"><a href="/sosig/event/list.do" title="진행중인 이벤트 선택됨">진행중인 이벤트</a></li>
		...
		<div class="list_searchbox">
			...
			<button class="searchBtn" type="button" onclick="fnSelectInfs(1)"><span class="blind">진행중인 이벤트 검색</span><i class="fa fa-search" aria-hidden="true" ></i></button>
		...
		<div class="gallery_list" id="menu1">
			<ul class="floats">
				<c:forEach var="item" items="${event_list}">
				<li>
					<c:choose>
					...
					<a href="#;" onclick="view2('${item.evn_no}')" class="subject">
						...
						<em>
							${fn:substring(item.evn_subject,0,13)}
							<c:if test="${fn:length(item.evn_subject) > 12 }">...</c:if>
						</em>
...
function fnSelectInfs(pageIndex) {
	...
	$("#frm").attr("action", "/sosig/event/list.do");
	$("#frm").submit();
