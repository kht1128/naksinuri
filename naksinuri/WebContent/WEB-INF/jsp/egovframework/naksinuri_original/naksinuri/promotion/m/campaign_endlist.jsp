...
<c:set var="pagesize" value="${select_total/pageUnit}"/>
	...
	<div id="fishjobList" class="content respon2">
		<div class="tabArea"> <!-- 탭이 3개면 class="tabArea tab3" 4개면 뭐겠냐 -->
			<ul class="floats">
					<li><a href="/promotion/campaign/m/list.do">진행중인 캠페인</a></li>
					<li class="on"><a href="/promotion/campaign/m/endlist.do">종료된 캠페인</a></li>
			</ul>
		</div>
					...
					<tbody>
						...
						<c:forEach var="item" items="${select_list}">
							<tr>
								...
								</i> ${fn:substring(item.bo_start_dt,0,10) }~${fn:substring(item.bo_end_dt,0,10) }</span>
...
function fnSelectInfs(pageIndex) {
	...
	$("#frm").attr("action", "${pageContext.request.contextPath}/promotion/campaign/m/endlist.do");
	$("#frm").submit();
