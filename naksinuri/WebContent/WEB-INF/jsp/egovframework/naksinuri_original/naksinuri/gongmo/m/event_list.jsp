	...
	<c:set var="pagesize" value="${event_total/pageUnit}"/>
		...
		<div id="fishjobList" class="content respon2">
			...
			<div class="tabArea tab3">
				<ul class="floats">
					<li class="on"><a href="/gongmo/event/m/list.do" id="event" >진행중인 이벤트</a></li>
					<li><a href="/gongmo/endevent/m/list.do" id="endevent">종료된 이벤트</a></li>
					<li><a href="/gongmo/ancevent/m/list.do" id="ancevent">공지사항 & 당첨자발표</a></li>				
				</ul>
			</div>
			...
			<div class="board_list">
				<table class="list_tbl">
					...
					<c:forEach var="item" items="${event_list}">
						<tr>
							<td class="textcenter"><a href="#;" onclick="view2('${item.evn_no}')"><c:out value="${item.rn}"></c:out></a></td>
					...
					<li>
						...
						<c:choose>
							<c:when test="${item.orignl_file_nm ne null }">
								<a href="#;" onclick="view2('${item.evn_no}')" class="pic"><img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${item.evn_main_img}"/>&fileSn=<c:out value="${item.file_sn }"/>'  width="100%" /></a>
							</c:when>
							<c:otherwise>
								<a href="#;" onclick="view2(${item.evn_no})" class="pic"><img src="/naksinuri_original/common_main/img/noImage_big.png" alt='<c:out value="낚시누리"/>' width="100%"/></a>
							</c:otherwise>
						</c:choose>
						<%-- 
						<a href="#;" onclick="view2('${item.evn_no}')" class="subject">
							<span class="cate">진행중인 이벤트</span>
							<c:set var="TextValue" value="${item.evn_subject}"/>
...
if(pageName =='event' && gallery_list == 'gallery'){
	...
	function fnSelectInfs(pageIndex) {
		...
		$("#frm").attr("action", "/gongmo/event/m/list.do");
		$("#frm").submit();
