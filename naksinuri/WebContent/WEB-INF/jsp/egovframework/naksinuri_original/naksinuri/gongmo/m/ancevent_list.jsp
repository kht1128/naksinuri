		...
		<div id="fishjobList" class="content respon2">
			...
			<div class="tabArea tab3">
				<ul class="floats">
					...
					<li class="on"><a href="/gongmo/ancevent/m/list.do" id="ancevent">공지사항 & 당첨자발표</a></li>				
				</ul>
			</div>
					...
					<c:forEach var="item" items="${ancevent_list}">
						<tr>
							...
							<td class="subject align_left">
								<a href="#;"onclick="view2('${item.evn_no}')">
									<c:set var="TextValue" value="${item.evn_subject}111122223333444455556666"/>
									<em>${fn:substring(TextValue,0,45)}<c:if test="${fn:length(TextValue) > 46 }">...</c:if></em>
									<span class="date">${fn:substring(item.evn_insert_dt,0,10)}</span>
									<span class="date"> | 조회수 : <c:out value="${item.evn_viewhit }"></c:out></span>
								</a>
							</td>
							<!-- 
...
<script>
	...
	function fnSelectInfs(pageIndex) {
		...
		$("#frm").attr("action", "${pageContext.request.contextPath}/gongmo/ancevent/m/list.do");
		$("#frm").submit();
