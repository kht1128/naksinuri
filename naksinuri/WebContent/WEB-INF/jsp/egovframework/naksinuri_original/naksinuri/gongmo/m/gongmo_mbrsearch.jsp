...
<%@include file="../../layout/m/head.jsp"%>
	...
	<div id="customerSound" class="content respon3">
		...
		<div class="tabArea tab2"> <!-- 탭이 3개면 class="tabArea tab3" 4개면 뭐겠냐 -->
			<ul class="floats">
				<li><a href="/gongmo/gongmo/m/check.do">공모전 접수/신청</a></li>
		...
		<section id="writeForm" class="write_box">
			...
			<c:if test="${info ne null}">
				...
				<dl>
					...
					<dd><input type="text" class="write_input w100" placeholder="이름을 입력해주세요." id="mbr_name" name="mbr_name" autocomplete="off" /></dd>
				</dl>
				<dl>
					<dt><span class="colorRed">*</span> 연락처</dt>
					<dd><input type="text" class="write_input w100" placeholder="전화번호를 입력해주세요. (ex:010-1234-1234)" id="mbr_hp" name="mbr_hp" autocomplete="off" /></dd>
				</dl>
				<dl>
					<dt><span class="colorRed">*</span> 암호</dt>
					<dd>
						<input type="password" class="write_input w100" placeholder="암호를 입력해주세요." id="mbr_pwd" name="mbr_pwd" autocomplete="off"/>
						<span class="txt_red">
...
function search_gongmo(){
		...
		form.action = "/gongmo/gongmo/m/mbrconfirm.do";
		form.submit();
}

//공모전 선택
function change_gongmo(obj) {
	$("#listform").attr("action", "/gongmo/gongmo/m/search.do");
	$("#listform").submit();
}
