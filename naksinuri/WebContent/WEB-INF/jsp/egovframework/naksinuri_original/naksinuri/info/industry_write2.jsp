...
<c:if test="${info.san_sn eq NULL}">
	<c:set var="action_file" value="/info/industry/ind_insert.do" />
</c:if>
<c:if test="${info.san_sn ne NULL}">
	<c:set var="action_file" value="/info/industry/ind_update.do" />
</c:if>

<form:form commandName="supform" name="supform" id="supform" method="post" enctype="multipart/form-data" >
...
<P CLASS=HStyle0 STYLE='text-align:right;line-height:300%;'><SPAN STYLE='font-size:12.0pt;font-family:"맑은 고딕";font-weight:"bold";color:#282828;line-height:300%'>본인은 상기 내용과 같이 개인정보를 수집⦁이용하는데 동의합니다.</SPAN></P>			
		...
		--%>
			...
			<dl>
				<dt>상호명</dt>
				<dd><input type="text" class="write_input w50" name="san_name" id="san_name" placeholder="예)낚시누리낚시점"  value="${info.san_name}"  /></dd>
			</dl>
			<dl>
				<dt>대표자</dt>
				<dd><input type="text" class="write_input w50" name="san_buisnessman" id="san_buisnessman"  value="${info.san_buisnessman}" placeholder="예)누리지기" /></dd>
			</dl>
			<dl>
				<dt>회사소개</dt>
			...
			<dl>
				<dt>사업영역</dt>
				<dd><input type="text" class="write_input w70" name="san_item" id="san_item" placeholder="ex) 낚시대 제조, 판매, 출조정보 제공" value="${info.san_item}" /></dd>
			</dl>
			<dl>
				<dt>홈페이지</dt>
				<dd><input type="text" class="write_input w70" name="san_homepage" id="san_homepage" value=<c:choose><c:when test="${info.san_homepage eq null }">"http://"</c:when><c:otherwise>"${info.san_homepage}"</c:otherwise></c:choose>  /></dd>
			</dl>
			<dl>
				<dt>일반전화</dt>
				<dd>
					...
					<input type="text" class="write_input w100px" name="san_tel2" id="san_tel2" maxlength="4" value="${info.san_tel2}" /> - 
					<input type="text" class="write_input w100px" name="san_tel3" id="san_tel3" maxlength="4" value="${info.san_tel3}" />
			...
			<dl>
				...
				<dd>
					...
					<input type="text" class="write_input w100px" id="co_hphone2" name="san_phone2" value="${info.san_phone2}"   maxlength="4" /> - 
					<input type="text" class="write_input w100px" id="co_hphone3" name="san_phone3" value="${info.san_phone3}"  maxlength="4" />
			...
			<c:choose>
					<c:when test="${info.san_sn ne null }">
						<div id="btnArea">
							<button type="button" class="btn_request btn_blue" id="industy_fsubmit">수정하기</button>
					</c:when>
					<c:otherwise>
					<%--
						<dl>
					...
					--%>
						<div id="btnArea">
							<button type="button" class="btn_request btn_blue" id="industy_fsubmit">등록 신청</button>
					</c:otherwise>
			</c:choose>	
				<button type="button" class="btn_prev btn_blue" id="preview" >미리보기</buttopn>
				<button type="button" class="btn_prev btn_white" onclick="cancel()">취소</button>
			</div>
		</section>
	</div>
</form:form>
...
nhn.husky.EZCreator.createInIFrame({
    ...
    fOnAppLoad : function(){
    	//웹접근성대응 oEditors.getById["san_content"].exec("PASTE_HTML", [" "]);
    },
...
function pasteHTML(filepath){
    ...
    //웹접근성대응 oEditors.getById["san_content"].exec("PASTE_HTML", [sHTML]);
   
}

$("#industy_fsubmit").click(function(){
	//웹접근성대응 oEditors.getById["san_content"].exec("UPDATE_CONTENTS_FIELD", [ ]);
	 /*
	var ischk1 = false;
	...
	*/
	...
	if($('#san_sn').val()!=''){
		$('#supform').attr('action','/info/industry/ind_update.do');
	}else{
		$('#supform').attr('action','/info/industry/ind_insert.do');
	}*/
	$('#save_status').val('r');
	$("#supform").attr("target","parent_window");	
	$("#supform").attr("action","/info/insertCorpSanupPreview.do");
	$('#supform').submit();
 });
