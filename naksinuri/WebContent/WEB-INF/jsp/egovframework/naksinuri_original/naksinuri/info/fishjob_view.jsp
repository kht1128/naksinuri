	...
	<div id="fishjobView" class="content respon2">	
		...
		<div id="tide"></div>
			...
			<div class="fish_slide">
					...
					<li>
						<c:choose>
					 		<c:when test="${(mimg.co_mimgsrc eq null) and (fn:length(filelist) eq 0)  }">
					 			<img alt="이미지 없음" src="/naksinuri_original/common_main/img/noImage_big.png">
					 		</c:when>
					 		<c:otherwise>
								<!--
								...
								-->
								<img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${mimg.co_mimgsrc}"/>&fileSn=<c:out value="${mimg.file_sn}"/>' style="margin-left:auto; margin-right: auto;"  height="510" alt="${info.co_nm} 사진" />
					 		</c:otherwise>
				 		</c:choose>
					</li>
						<c:forEach var="images" items="${filelist}" varStatus="status">
							<li>
								...
								-->
								<img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${images.atch_file_id}"/>&fileSn=<c:out value="${images.file_sn}"/>' style="margin-left:auto; margin-right: auto;"  height="510" alt="${info.co_nm} 사진" />
				...
				<script>
					$('#basicInfo ul').bxSlider({
						...
						nextText: '<i class="fa fa-angle-right" title="다음"></i>',
						prevText: '<i class="fa fa-angle-left" title="이전"></i>',
						pause: 5000
					});
				</script>
			</div>
			<div class="fishshop_info">
				...
				<h1><span class="cate"><c:if test="${info.co_addr2_2 ne null ||  info.co_addr2_2 ne '' }"> [${sido[0]} ${sido[1]}]</c:if></span><em>${info.co_nm}</em></h1>
				<div class="info">
					<c:choose>
						<c:when test="${info.fishing_type eq 'boatfishing'  }">
							...
							<c:if test="${info.co_phone3 ne ''}">
								<dl><dt>일반전화</dt><dd><c:if test="${info.co_phone1 ne ''}">${info.co_phone1}-</c:if>${info.co_phone2}-${info.co_phone3}</dd></dl>
							</c:if>
							<dl><dt>휴대전화</dt><dd><c:if test="${(info.co_hphone2 ne '')&& (info.co_hphone3 ne '') }">${info.co_hphone1}-${info.co_hphone2}-${info.co_hphone3}</c:if></dd></dl>
							<c:if test="${!empty info.co_2hphone3}">
								<dl><dt>휴대전화2</dt><dd><c:if test="${info.co_2hphone1 ne ''}">${info.co_2hphone1}-</c:if>${info.co_2hphone2}-${info.co_2hphone3}</dd></dl>
							</c:if>
							<dl><dt>낚시업종</dt><dd>선상낚시</dd></dl>
							<dl><dt>어선규모</dt><dd><c:if test="${info.bo_size ne '' }">${info.bo_size}톤</c:if></dd></dl>
							<dl><dt>어선속도</dt><dd><c:if test="${info.bo_spd ne ''}">${info.bo_spd}knot</c:if></dd></dl>
							<dl><dt>승선인원</dt><dd><c:if test="${info.bo_psg ne ''}">${info.bo_psg}명</c:if></dd></dl>
							<dl><dt>이용시간</dt><dd><c:if test="${ info.co_stm ne ''}">${info.co_stm} </c:if><c:if test="${info.co_etm ne ''}"> ~ ${info.co_etm}</c:if></dd></dl>
							<dl><dt>카드결제</dt><dd><c:if test="${info.co_credit eq '0'}">불가</c:if><c:if test="${info.co_credit eq '1'}">가능</c:if></dd></dl>
						    <c:choose>
							   	<c:when test="${info.co_web eq 'http://' }"> </c:when>
							  		<c:otherwise>
										<dl><dt>홈페이지</dt><dd><a href="${info.co_web}" target="_blank">${info.co_web}</a></dd></dl>
									</c:otherwise>
							</c:choose>
						...
						<c:when test="${info.fishing_type eq 'riverfishing' || info.fishing_type eq 'seafishing' }">
							...
							<dl><dt>낚시업종</dt><dd><c:if test="${info.co_info ne '' }">${info.co_info}</c:if></dd></dl>
							<dl><dt>수용인원</dt><dd><c:if test="${ info.co_psg ne ''}">${info.co_psg}명</c:if></dd></dl>
							<dl><dt>이용시간</dt><dd><c:if test="${ info.co_stm ne ''}">${info.co_stm} </c:if><c:if test="${info.co_etm ne ''}"> ~ ${info.co_etm}</c:if></dd></dl>
							<dl><dt>카드결제</dt><dd><c:if test="${info.co_credit eq '0'}">불가</c:if><c:if test="${info.co_credit eq '1'}">가능</c:if></dd></dl>
							<c:choose>
								<c:when test="${info.co_web eq 'http://' or info.co_web eq ''}"></c:when>
								<c:otherwise>
									<dl><dt>홈페이지</dt><dd><a href="${info.co_web}" target="_blank">${info.co_web}</a></dd></dl>
								</c:otherwise>
							</c:choose>
						...
						<c:otherwise>							
							...
							<dl><dt>낚시업종</dt><dd><c:if test="${info.co_info ne ''  }">${info.co_info}</c:if></dd></dl>
							<dl><dt>수용인원</dt><dd><c:if test="${info.bo_psg ne ''}">${info.co_psg}명</c:if></dd></dl>
							<dl><dt>이용시간</dt><dd><c:if test="${info.co_stm ne ''}">${info.co_stm} </c:if><c:if test="${info.co_etm ne ''}"> ~ ${info.co_etm}</c:if></dd></dl>
							<dl><dt>카드결제</dt><dd><c:if test="${info.co_credit eq '0'}">불가</c:if><c:if test="${info.co_credit eq '1'}">가능</c:if></dd></dl>
		...
		<section id="majorFish" class="icon_box">
			...
			<ul class="floats" id="fishul">
				...
				<c:forEach var="fishes" items="${fishlist}" varStatus="status">
					...
					<c:if test ="${fn:contains(info.co_fish,subname)}">
		...
		<section id="majorWeather" class="icon_box">
			<h2>${info.co_addr2_2.substring(0,2)} 날씨/물때</h2>
			<ul class="floats">
				<li id="li_day1">
					<div class="weather">
		...
		<section id="mapArea">
			<div class="tabArea">
				<ul class="floats">
					<li class="on">
						<a href="#;" onclick="locate()" id="locate" class="map_tab_title" title="오시는 길 선택됨">
						<i class="fa fa-map-marker" aria-hidden="true" title="오시는길"></i> 오시는 길
						</a>
					</li>
					
					<li>
						<a href="#;" onclick="public_festival()" id="public_festival" class="map_tab_title" title="축제/행사">
							<i class="fa fa-flag" aria-hidden="true" title="축제/행사"></i> 축제/행사
						</a
					</li>
					<li>
						<a href="#;" onclick="public_trv()" id="public_trv" class="map_tab_title" title="관광지">
							<i class="fa fa-map" aria-hidden="true" title="관광지"></i> 관광지
						</a>
					</li>
					<li>
						<a href="#;" onclick="public_food()" id="public_food" class="map_tab_title" title="음식점">
							<i class="fa fa-cutlery" aria-hidden="true" title="음식점"></i> 음식점
						</a>
					</li>
					<li>
						<a href="#;" onclick="public_shop()" id="public_shop" class="map_tab_title" title="쇼핑">
							<i class="fa fa-shopping-bag" aria-hidden="true" title="쇼핑"></i> 쇼핑
						</a>
					</li>
				</ul>
			</div>
		...
		<div id="btnArea">
			<ul class="floats">
				<li>
					<a href="#" onclick="go_prev(${prev.nak_id});" class="btn_prev btn_white" title="이전글"><i class="fa fa-angle-left" aria-hidden="true" title="이전글"></i> 이전글</a>
					<a href="#" onclick="go_next(${next.nak_id});" class="btn_next btn_white" title="다음글">다음글 <i class="fa fa-angle-right" aria-hidden="true" title="다음글"></i></a>
				</li>
				<li class="fr">
					<a href="#;" onclick="go_review()" class="btn_review btn_blue" title="낚시터 후기">낚시터 후기</a>
					<a href="#;" onclick="fishjob_report()" class="btn_report btn_red" title="잘못된 정보 신고">잘못된 정보 신고</a>
					<a href="/info/fishjob/list.do" class="btn_list btn_gray" title="목록">목록</a>
				</li>
			</ul>
		</div>
			...
			<div class="reply_list">
				<c:forEach var="rev" items="${review_list}">
					<div class="reply_con">
						...
						<span class="date">작성자:<c:choose>
							<c:when test="${fn:length(rev.rev_writer) < 2}">
								*${rev.rev_writer}*
							</c:when>
							<c:when test="${fn:length(rev.rev_writer) < 3}">
								${fn:substring(rev.rev_writer,0,1)}*
							</c:when>
							<c:otherwise>
								${fn:substring(rev.rev_writer,0,1)}<c:forEach begin="2" end="${fn:length(rev.rev_writer)-1}" step="1">*</c:forEach>${fn:substring(rev.rev_writer,fn:length(rev.rev_writer)-1,fn:length(rev.rev_writer))}
							</c:otherwise>
						</c:choose></span>
...
// 	지도 탭제어	
		...
		$('.map_tab_title').on('focusin',function(event){
			var map_title_id = $(this).attr('id');
			if( map_title_id == 'locate' ){
				$('#locate').attr('title','오시는 길 선택됨');
				$('#public_festival').attr('title','축제/행사');
				$('#public_trv').attr('title','관광지');
				$('#public_food').attr('title','음식점');
				$('#public_shop').attr('title','쇼핑');
			...
			else if( map_title_id == 'public_festival' ){
				$('#locate').attr('title','오시는 길');
				$('#public_festival').attr('title','축제/행사 선택됨');
				$('#public_trv').attr('title','관광지');
				$('#public_food').attr('title','음식점');
				$('#public_shop').attr('title','쇼핑');				
				
				$('#map_title').html('축제/행사');
			...
			else if( map_title_id == 'public_trv' ){
				$('#locate').att('title','오시는 길');
				$('#public_festival').att('title','축제/행사');
				$('#public_trv').att('title','관광지 선택됨');
				$('#public_food').att('title','음식점');
				$('#public_shop').att('title','쇼핑');
			...
			else if( map_title_id == 'public_food' ){
				$('#locate').att('title','오시는 길');
				$('#public_festival').att('title','축제/행사');
				$('#public_trv').att('title','관광지');
				$('#public_food').att('title','음식점 선택됨');
				$('#public_shop').att('title','쇼핑');
			...
			else if( map_title_id == 'public_shop' ){
				$('#locate').att('title','오시는 길');
				$('#public_festival').att('title','축제/행사');
				$('#public_trv').att('title','관광지');
				$('#public_food').att('title','음식점');
				$('#public_shop').att('title','쇼핑 선택됨');
		...
		$("#public_festival").click(function(){
			...
			$('#public_trv').attr('title','관광지');
			$('#public_food').attr('title','음식점');
			$('#public_shop').attr('title','쇼핑 선택됨');
			
			$('#map_title').html('축제/헹사');
			$("#locate").parent().removeClass("on");
			$("#public_festival").parent().addClass("on");
			$("#public_trv").parent().removeClass("on");
			$("#public_food").parent().removeClass("on");
		...
		$("#public_food").click(function(){
			...
			$('#public_festival').attr('title','축제/행사');
			$('#public_trv').attr('title','관광지');
			$('#public_food').attr('title','음식점 선택됨');
			$('#public_shop').attr('title','쇼핑');
