		...
		<div class="leftconBox">
			<section id="basicInfo">
				<div class="business_logo">
					<img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${info.san_img}"/>&fileSn=<c:out value="${info.file_sn}"/>' alt=<c:out value="${info.san_name}"/> />
				</div>
			...
			<div class="businessSlide">
					<ul>
						<c:forEach var="simges" items="${simglist}" varStatus="status">
							<li style="text-align:center;background-color:#f3f3f3"><img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${simges.san_simg}"/>&fileSn=<c:out value="${simges.file_sn}"/>' alt='<c:out value="${simges.file_cn}"/>' style="width:auto;height:540px;margin:0 auto;" /></li>
						</c:forEach>
					</ul>
					<script>
