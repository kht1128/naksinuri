...
<form:form commandName="supform" id="supform" name="supform" method="post" enctype="multipart/form-data" >
	...
	<div id="businessWrite" class="content respon3">
		<section id="writeForm" class="write_box">
			...
			<dl>
				<dt>상호명</dt>
				<dd><input type="text" size="50" class="write_input" name="san_name" id="san_name" placeholder="예)낚시누리낚시점"  value="${info.san_name}"  /></dd>
			</dl>
			<dl>
				<dt>대표자</dt>
				<dd><input type="text" size="50" class="write_input" name="san_buisnessman" id="san_buisnessman"  value="${info.san_buisnessman}" placeholder="예)누리지기" /></dd>
			</dl>
			<dl>
				<dt>회사소개</dt>
			...
			<dl>
				<dt>사업영역</dt>
				<dd><input type="text" size="100" class="write_input" name="san_item" id="san_item" placeholder="ex) 낚시대 제조, 판매, 출조정보 제공" value="${info.san_item}" /></dd>
			</dl>
			<dl>
				<dt>홈페이지</dt>
				<dd><input type="text" size="100" class="write_input" name="san_homepage" id="san_homepage" value=<c:choose><c:when test="${info.san_homepage eq null }">"http://"</c:when><c:otherwise>"${info.san_homepage}"</c:otherwise></c:choose>  /></dd>
			</dl>
			<dl>
				<dt>일반전화</dt>
				<dd>
			...
			<dl>
				...
				<dd class="file_input">
					...
					<c:if test="${info.orignl_file_nm eq null }">
							...
							<c:forEach items="${filelist}" var="item">
								<li>
									<c:out value="${item.orignl_file_nm}"/>&nbsp;<c:out value="[${item.file_size} Byte]"/>
					       			...
					       			<label for="delcheck_${item.san_simg}_${item.file_sn}"><span></span>삭제</label>
								</li>
							</c:forEach>
							<li id="imgli_0">
								<input type="checkbox" id="chart_0"/><label for="chart_0"><span></span></label> 
								<input type="file" size="50" name="bo_file_0" class="write_file" />
							</li>
						</ul>
					</div>
					<div id="btnArea">
			...
			<c:choose>
					<c:when test="${info.san_sn ne null }">
						<div id="btnArea">
							<button type="button" class="btn_request btn_blue" id="industy_fsubmit">수정하기</a>
					</c:when>
					<c:otherwise>
						<dl>
							<dt>자동등록방지</dt>
				...
				<button type="button" class="btn_prev btn_blue" id="preview" >미리보기</button>
