...
<div id="container">
	...
	<div id="fishjobView" class="content respon2">	
		...
		<center><div style="padding-top: 30px; font-size: 48px; font-weight: bold; letter-spacing: -0.5pt; font-family: NanumSquare;">미리보기</div></center>
		...
		<div id="tide"></div>
		<%--
		pmimg.co_mimgsrc : ${pmimg.co_mimgsrc}
		mimg.co_mimgsrc : ${mimg.co_mimgsrc}
		 --%>
		 	<div class="fish_slide">
				<ul>
					<li>
						<c:choose>
					 		<c:when test="${(pmimg.co_mimgsrc ne null) and (fn:length(prefilelist) ne 0)  }">
					 		...
					 		<c:when test="${(mimg.co_mimgsrc ne null) and (fn:length(prefilelist) ne 0)  }">
						...
						<c:forEach var="images" items="${prefilelist}" varStatus="status">
							<li>
								<img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${images.p_atch_file_id}"/>&fileSn=<c:out value="${images.p_file_sn}"/>' style="margin-left:auto; margin-right: auto;"  height="510" alt=<c:out value="${images.p_orignl_file_nm}" /> />
							</li>
						</c:forEach>
				</ul>
				<div class="outarrow">
			...
			<div class="fishshop_info">
				...
				<h1><span class="cate"><c:if test="${preview.co_addr2_2 ne null ||  preview.co_addr2_2 ne '' }"> [${psido[0]} ${psido[1]}]</c:if></span><em>${preview.co_nm}</em></h1>
				<div class="info">
					<c:choose>
