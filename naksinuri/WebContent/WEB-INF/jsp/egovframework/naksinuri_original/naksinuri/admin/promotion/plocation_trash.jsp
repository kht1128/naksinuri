...
<%@include file="../admin_leftTab.jsp"%>
	<div id="container">
		...
		<div id="content">			
			...
			<h1 class="bigTit"><i class="fa fa-trash" aria-hidden="true"></i> 낚시금지구역 휴지통 <div class="trash_box"><%@include file="../trash_selectBox.jsp" %></div></h1>
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${select_list}" varStatus="status">
							<tr>
								...
								<td class="textcenter"><a href="#;" onclick="view2('${item.x_sn}')">${item.x_term}</a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.x_sn}')">${item.x_rang}</a></td>
...
function fncGroupListDelete() {
	if(fncManageChecked()) {
	    if(confirm("삭제하시겠습니까?")) {
            document.listform.action = "./delete_list_p.do";
...
function fncGroupListRestore() {
	if(fncManageChecked()) {
	    if(confirm("복구하시겠습니까?")) {
            document.listform.action = "./retore_p.do";
            document.listform.submit();
	    }
	}
}
