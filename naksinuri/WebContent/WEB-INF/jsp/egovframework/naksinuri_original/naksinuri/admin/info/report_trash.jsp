...
<div id="wrapper">
	...
	<div id="container">
		...
		<div id="content">			
					...
					<h1 class="bigTit"><i class="fa fa-trash" aria-hidden="true"></i> 낚시터 잘못된 정보 신고 휴지통 <div class="trash_box"><%@include file="../trash_selectBox.jsp" %></div></h1>
							...
							<tr>
								...
								<td colspan="5">
									<select class="frm_select" style="width:100px" name="searchType">
										...
										<option value="report_writer">글쓴이</option>
		...
		<form:form commandName="listform" id="listform" name="listform" method="post" enctype="multipart/form-data" >
				...
				<input type="hidden" name="report_sn" id="report_sn"/>
				<!-- 리스트 영역 { -->
				<div class="t_list_area">
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${select_list}">
							<tr>
								<td><input type="checkbox" name="delYn" class="check2" value="${item.report_sn}"></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.report_sn}')"><c:out value="${select_total-item.rn + 1}"></c:out></a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.report_sn}')">${item.report_writer}</a></td>
								<td><a href="#;" onclick="view2('${item.report_sn}')">${item.report_subject}</a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.report_sn}')">
									<fmt:parseDate value="${item.report_date}" var="f_data" pattern="yyyy-MM-dd HH:mm:ss.SSS" scope="page"/>
