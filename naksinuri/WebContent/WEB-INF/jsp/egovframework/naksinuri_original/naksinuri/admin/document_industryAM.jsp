...
<c:set var="pageName" value="sanupAM" />
...
<%@include file="admin_leftTab.jsp"%>
	<div id="container">
		...
		<div id="content">			
			...
			<input type="hidden" name="bo_sns" id="bo_sns"/>
				...
				<div class="t_list_area">
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${admin_list}">
							<c:choose>
								<c:when test="${fn:length(item.san_sn) > 8}">
							...
							<tr>
								...
								<td><input type="checkbox" name="delYn" class="check2" value="${item.san_sn}"></td>
								 --%>
								<td class="textcenter"><a href="#;" onclick="preview_admin('${item.san_sn}','${item.preg_pid}')">${fdata}</a></td>
								<td class="textcenter"><a href="#;" onclick="preview_admin('${item.san_sn}','${item.preg_pid}')">${item.san_buisnessman}</a></td>
								<td class="textcenter"><a href="#;" onclick="preview_admin('${item.san_sn}','${item.preg_pid}')">${item.san_name}</a></td>
								<td><a href="#;" onclick="preview_admin('${item.san_sn}','${item.preg_pid}')">${item.san_address2}</a></td>
								<td class="textcenter"><a href="#;" onclick="preview_admin('${item.san_sn}','${item.preg_pid}')">${fn:substring(item.san_date,0,16)}</a></td>
				...
				<!-- } 리스트 영역 -->
				<%--
				<div align="right"><a href="#;" onclick="fncGroupListPreviewDelete()" class="btn_size1 btn_violet">삭제</a></div>
				 --%>
			</section>
		</div>
	</div>
	...
	<!-- } 하단 푸터 -->
	<form:form commandName="supform2" name="supform2" id="supform2" method="post" >
		<input type="hidden" name="san_sn" id="san_sn" value="">
	</form:form>
<script>
function original_view(pid){	
	var form = document.getElementById('seaform');
	$('#san_sn').val(pid);	
	form.action="/admin/info/industry/ind_find.do";
	var gsWin = window.open("about:blank", "winName");
	form.target = "winName";
	form.submit();	
};

function preview_admin(san_sn,pid){
	window.name = "admin_page";
	/*
	$('#san_sn').val(san_sn);
    var openwin = window.open('','admin_preview', 'width=1400, height=800, scrollbars=yes, resizable=no');
    $("#supform2").attr("target","admin_preview");
    $("#supform2").attr("action","/info/admin/indPreview.do");	
	$('#supform2').submit();
	openwin.focus();
	*/
    var openwin = window.open('/info/admin/indPreview.do?san_sn='+san_sn+'&pid='+pid,'admin_preview', 'width=1400, height=800, scrollbars=yes, resizable=yes');
    openwin.focus();
