						...
						<c:forEach var="item"  items="${select_list }" varStatus="status">
							<tr>
								...
								<td class="textcenter"><a href="#;">
									<fmt:parseDate value="${item.qna_insert_dt}" var="f_data" pattern="yyyy-MM-dd HH:mm:ss.SSS" scope="page"/>
									<fmt:formatDate value="${f_data}" pattern="yyyy-MM-dd HH:mm:ss"/>
								</a></td>		
								<td class="textcenter"><a href="#;">
									<fmt:parseDate value="${item.qna_update_dt}" var="f_data" pattern="yyyy-MM-dd HH:mm:ss.SSS" scope="page"/>
									<fmt:formatDate value="${f_data}" pattern="yyyy-MM-dd HH:mm:ss"/>
								</a></td>		
...
function fncGroupListDelete() {
	if(fncManageChecked()) {
	    if(confirm("삭제하시겠습니까?")) {
            document.listform.action = "./gotrash.do";
