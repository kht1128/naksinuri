						...
						<div id="search_form">
							<table class="t_search">
								...
								<tbody>
									...
									<tr>
										<th>검색</th>
										<td colspan="5"><select class="frm_select"
											style="width: 100px" name="searchType">

												<option value="bo_subject">제목</option>
												<option value="bo_name">글쓴이</option>

										</select> <input type="text" class="frm_input" size="70"
											name="searchText" />
											<button class="btn_size1 btn_violet" type="button"
												onclick="fnSelectInfs(1);">검색</button></td>
									</tr>
								</tbody>
							</table>
						</div>
...
<form:form commandName="listform" id="listform" name="listform" method="post" enctype="multipart/form-data">
	...
	<div class="t_list_area">
		<div class="listtop floats">
			...
			<div class="listnum">
				출력수 <select class="frm_select" id="s_pageUnit" name="s_pageUnit"
					onchange="fnSelectInfs(1);">
				...
				<c:forEach var="item" items="${select_list}">
					<tr>
						...
						<td class="textcenter"><a href="#;" onclick="view2('${item.bo_sn}')"> 
						  <c:choose>
						    	<c:when test="${fn:length(item.bo_name)>7 }">${fn:substring(item.bo_name,0,8)}...</c:when>
								<c:otherwise>${item.bo_name}</c:otherwise>
					     </c:choose>
	...
	<footer id="footer" class="floats">
		...
		<div class="r_copyright">
			<b>Endpoint Protector 4</b> Copyright 2004-2016 CoSoSys Ltd. All
			rights reserved.
		</div>
	</footer>
	<!-- } 하단 푸터 -->
