...
<div id="container">
	...
	<div id="fishjobView" class="content respon2">	
		...
		<input type="hidden" id="preg_pid" name="preg_pid" value="${preview.preg_pid}">
				...
				<ul>
					<li>
						<c:choose>
					 		...
					 		<c:when test="${(info.co_mimgsrc ne null) and (fn:length(prefilelist) ne 0)  }">
								<img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${info.co_mimgsrc}"/>&fileSn=<c:out value="${porgmimg.file_sn}"/>' style="margin-left:auto; margin-right: auto;"  height="510" alt=<c:out value="${porgmimg.orignl_file_nm}" />/>
					 		</c:when>
					 		<c:otherwise>
		...
		<div id="btnArea">
			<button type="button" class="btn_review btn_blue" id="opener_save">적용</button>
			<button type="button" onclick="self.window.close();" class="btn_review btn_blue">취소</button>
			<button type="button" onclick="delpreview()" class="btn_review btn_red">삭제</button>
		</div>
	</div>
</div>
...
function delpreview() {
	if(confirm('등록된 미리보기를 정말 삭제하시겠습니까?\n\n삭제한 데이터는 복구가 불가능합니다.')) {
		var nak_id = $('#nak_id').val();
		location.href = "/info/admin/fishPreviewdel.do?nak_id="+nak_id;
	}
}
...
$("#opener_save").click(function(){
	if(confirm('위 정보로 데이터를 저장하시겠습니까?')) {
		var nak_id = $('#nak_id').val();
		var preg_pid = $('#preg_pid').val();
		location.href="/info/admin/fishPreviewUpdate.do?nak_id="+nak_id+"&pid="+preg_pid;
	}
});
</script>
