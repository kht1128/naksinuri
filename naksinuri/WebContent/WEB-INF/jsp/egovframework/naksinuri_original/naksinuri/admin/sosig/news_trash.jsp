...
<div id="wrapper">
	...
	<div id="container">
		...
		<div id="content">			
			...
			<h1 class="bigTit"><i class="fa fa-trash" aria-hidden="true"></i> 낚시뉴스 휴지통 <div class="trash_box"><%@include file="../trash_selectBox.jsp" %></div></h1>
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${select_list}">
							<tr>
								...
								<td class="textcenter"><a target="_blank" href="${item.link}">${select_total-item.rn+1}</a></td>
								<td class="textcenter"><a target="_blank" href="${item.link}">${item.category}</a></td>
								<td class="textcenter"><a target="_blank" href="${item.link}">${item.writer_name}</a></td>
								<td><a target="_blank" href="${item.link}">${item.title}</a></td>
								<td class="textcenter"><a target="_blank" href="${item.link}">${item.regdate}</a></td>		
								<td class="textcenter"><a href="#;" onclick="view2('${item.mid}')"><button class="btn_size1 btn_bordergray">수정</button></a></td>	
								<td class="textcenter"><a target="_blank" href="${item.link}" onclick="view2('${item.mid}')">${item.isdel}</a></td>		
							</tr>
						</c:forEach>
						</tbody>
					</table>
