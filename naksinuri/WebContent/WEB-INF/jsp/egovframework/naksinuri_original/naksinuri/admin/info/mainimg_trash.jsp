...
<%@include file="../admin_leftTab.jsp"%>
	...
	<div id="container">
		...
		<div id="content">			
			...
			<h1 class="bigTit"><i class="fa fa-trash" aria-hidden="true"></i> 사용자메인이미지 설정 휴지통 <div class="trash_box"><%@include file="../trash_selectBox.jsp" %></div></h1>
							...
							<tr>
								...
								<td colspan="5">
									<select class="frm_select" style="width:100px" name="searchType">
										...
										<option value="img_subject">제목</option>
		...
		<form:form commandName="listform" id="listform" name="listform" method="post" enctype="multipart/form-data" >
			...
			<input type="hidden" name="img_no" id="img_no" value="${item.img_no }"/>
				<!-- 리스트 영역 { -->
					...
					<table class="t_list">
						...
						<thead>
							<tr>					
								...
								<th scope="col">이미지 표시 여부</th>
							</tr>
						</thead>
						<tbody>
						<c:forEach var="item" items="${select_list}">
							<tr>
								<td><input type="checkbox" name="delYn" class="check2" value="${item.img_no}"></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.img_no}')">${select_total-item.rn+1}</a></td>
								<td><a href="#;" onclick="view2('${item.img_no}')">${item.img_subject}</a></td>
								<td><a href="#;" onclick="view2('${item.img_no}')">${item.menu_category}</a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.img_no}')">
									<fmt:parseDate value="${item.updatedate}" var="f_data" pattern="yyyy-MM-dd HH:mm:ss.SSS" scope="page"/>
									<fmt:formatDate value="${f_data}" pattern="yyyy-MM-dd HH:mm:ss"/>
								</a></td>		
								<td class="textcenter"><a href="#;" onclick="view2('${item.img_no}')">${item.img_sta}</a></td>		
							</tr>
						</c:forEach>
						</tbody>
					</table>
