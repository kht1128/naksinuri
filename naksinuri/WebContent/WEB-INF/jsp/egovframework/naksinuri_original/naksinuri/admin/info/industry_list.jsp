	...
	<input type="hidden" name="san_sn" id="san_sn" value="${item.san_sn }"/>
				...
				<div class="t_list_area">
					<div class="listtop floats">
						<div class="totalnum">총 <b>${ind_total}</b>건</div>
						<div class="listnum">
							출력수 
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${ind_list}">
							<tr>
								...
								<td class="textcenter"><a href="#;" onclick="view2('${item.san_sn}')">${ind_total-item.rn+1}</a><br>(${item.san_sn})</td>
								...
								<td class="textcenter">
									<a href="#;" onclick="view2('${item.san_sn}')">
										<c:if test="${item.san_status eq 'n'}">미승인</c:if>
										<c:if test="${item.san_status eq 'y'}">승인</c:if>
										<c:if test="${item.san_status eq 'w'}">대기</c:if>
									</a>
								</td>
...
function fncGroupListDelete() {
	if(fncManageChecked()) {
	    if(confirm("삭제하시겠습니까?")) {
            document.listform.action = "./gotrash_ind.do";
