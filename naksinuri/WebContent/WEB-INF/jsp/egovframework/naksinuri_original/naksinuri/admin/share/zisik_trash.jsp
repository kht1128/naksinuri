...
<%@include file="../admin_leftTab.jsp"%>
	...
	<div id="container">
		...
		<div id="content">			
			...
			<h1 class="bigTit"><i class="fa fa-trash" aria-hidden="true"></i> 누리지식인 휴지통 <div class="trash_box"><%@include file="../trash_selectBox.jsp" %></div></h1>
							...
							<tr>
								...
								<td colspan="5">
									<select class="frm_select" style="width:100px" name="searchType">
										...
										<option value="nuri_q_subject"<c:if test="${searchType eq 'nuri_q_subject' }">selected</c:if>>제목</option>
										<option value="nuri_q_writer"<c:if test="${searchType eq 'nuri_q_writer' }">selected</c:if>>글쓴이</option>
	...
	<form:form commandName="listform" id="listform" name="listform" method="post" enctype="multipart/form-data" >
		...
		<input type="hidden" name="nuri_q_num" id="nuri_q_num" value="${item.nuri_q_num }"/>
				...
				<div class="t_list_area">
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${select_list}">
							<tr>
								<td><input type="checkbox" name="delYn" class="check2" value="${item.nuri_q_num}"></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.nuri_q_num}')">${select_total-item.rn + 1}</a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.nuri_q_num}')">${item.nuri_q_writer}</a></td>
								<td><a href="#;" onclick="view2('${item.nuri_q_num}')">${item.nuri_q_subject}</a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.nuri_q_num}')">
									<fmt:parseDate value="${item.nuri_q_date}" var="f_data" pattern="yyyy-MM-dd HH:mm:ss.SSS" scope="page"/>
...
function fncGroupListDelete() {
	if(fncManageChecked()) {
	    if(confirm("삭제하시겠습니까?")) {
            document.listform.action = "./deletez.do";
...
function fncGroupListRestore() {
	if(fncManageChecked()) {
	    if(confirm("복구하시겠습니까?")) {
            document.listform.action = "./restorez.do";
            document.listform.submit();
	    }
	}
}
