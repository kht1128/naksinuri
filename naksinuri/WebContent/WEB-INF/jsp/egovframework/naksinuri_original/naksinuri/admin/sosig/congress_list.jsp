	...
	<div id="container">
		...
		<div id="content">			
			...
			<section id="table-list">
				...
				<div id="tabarea">
					<ul class="floats">
						<li><a href="#;" class="cateing on">진행중인 행사(대회/박람회)</a></li>
						<li><a href="${pageContext.request.contextPath}/admin/sosig/congress/endlist.do" class="cateend">종료된 행사(대회/박람회)</a></li>
					</ul>
				</div>
				<!-- } 탭 영역 -->
		...
		<form:form commandName="listform" id="listform" name="listform" method="post" enctype="multipart/form-data" >
				...
				<input type="hidden" name="bo_email" id="bo_email" value=""/>
				<input type="hidden" name="regit_num" id="regit_num" value=""/>
					...
					<table class="t_list">
						...
						<c:forEach var="item2" items="${noticlist }">
							<tr>
								...
								<td class="textcenter">
									<c:choose>
									 	<c:when test="${item.is_entry_y eq '1'}"><a href="#;" <%-- onclick="show_applicant('${item.bo_sn}')" --%> class="txt_red show_applicant"
									 	data-bo-email="${item.bo_email }" data-regit-num="${item.regit_num}" data-bo-sn="${item.bo_sn}">모집중</a></c:when>								 
...
/* function show_applicant(bo_sn){
	...
	console.log($(this));
	var email = $(this).attr('data-bo-email');
	console.log(email);
	var regitNum = $(this).attr('data-regit-num');
	console.log(regitNum);
	
	form.action = url;
	form.method = "post";
	form.target="popwin";
	form.bo_sn = bo_sn;
	form.bo_email.value = email;
	form.regit_num.value = regitNum;	
	
	form.submit();
	
} */

$(".show_applicant").on("click", function(){
	var bo_sn = $(this).attr('data-bo-sn');
	var bo_email = $(this).attr('data-bo-email');
	var regit_num = $(this).attr('data-regit-num');
	...
	form.target="popwin";
	form.bo_sn = bo_sn;
	form.bo_email.value = bo_email;
	form.regit_num.value = regit_num;	
