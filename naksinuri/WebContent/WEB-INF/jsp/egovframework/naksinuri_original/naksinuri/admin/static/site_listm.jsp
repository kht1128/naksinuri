...
function getLessonURL(){
	...
	var url ="";
	url="/admin/getLessonURLmonth.do";
	var name="낚시교실 통계";
	chart(url,name);
};
...
function getPolicyURL(){
	...
	var url ="";
	url="/admin/getPolicyURLmonth.do";
	var name="정책홍보 통계";
	chart(url,name);
};
...
function getSurveyURL(){
	...
	var url ="";
	url="/admin/getSurveyURLmonth.do";
	var name="설문조사 통계";
	chart(url,name);
};
...
//for문 끝 
	...
	//for문 끝 
		...
		$('#end_dt').datepicker({dateFormat : 'yy-mm-dd'});
			...
			$.ajax({
				type:"GET",
				url :'/admin/getUser_BrowserInfo30.do',
				dataType: 'json',
				contentType: "application/json;charset=UTF-8",
				async: true,
				success: function(data, status, xhr) {
