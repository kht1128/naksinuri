	...
	<div id="container">
		...
		<div id="content">			
			...
			<section id="table-list">
				...
				<div id="tabarea">
					<ul class="floats">
						<li><a href="${pageContext.request.contextPath}/admin/gongmo/event/list.do" class="on">목록조회</a></li>
						...
						<li><a href="${pageContext.request.contextPath}/admin/gongmo/aevent/list.do" >공지사항 & 당첨자발표</a></li>
<!-- 						<li><a href="#;">상세내용</a></li> -->
					</ul>
				</div>
				<!-- } 탭 영역 -->
	...
	<form:form commandName="listform" id="listform" name="listform" method="post" enctype="multipart/form-data" >
		...
		<input type="hidden" name="evn_no" id="evn_no" value="${item.evn_no }"/>
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${event_list}">
							<tr>
								...
								<td class="textcenter txtred"><a href="#;" onclick="show_applicant('${item.evn_no}')" class="txt_red">참여자보기</a></td>	
							</tr>
						</c:forEach>
						</tbody>
					</table>
				...
				<div align="right"><a href="#;" onclick="fncGroupListDelete()" class="btn_size1 btn_violet">삭제</a><a href="/admin/gongmo/event/view.do" class="btn_size1 btn_violet">글쓰기</a></div>
				<!-- 페이지 { -->
				<div id="pagenation">
...
function show_applicant(evn_no){
	if(!$.trim(evn_no)){
		alert("해당 이벤트의 정보를 찾을 수 없습니다.");
		return false;
	}
	var form = document.getElementById('listform');
	$('#evn_no').val(evn_no);
	window.open("about:blank","popwin","width=1400,height=800,scrollbars=yes");
	form.action = "/gongmo/event/list_show.do";
	form.method = "post";
	form.target="popwin";
	form.submit();
