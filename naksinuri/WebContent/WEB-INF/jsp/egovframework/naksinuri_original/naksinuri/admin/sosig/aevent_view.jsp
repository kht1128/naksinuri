...
<input type="hidden" name="returnUrl" value="<c:url value='/admin/sosig/event/event_findCorp.do'/>"/>
...
<input type="hidden" name="evn_sta" id="evn_sta" value="A"/>
		...
		<div id="content">			
				...
				<div class="padding_box">
					<table class="t_write" id="ftable">
						...
						<tbody>
							...
							<tr>
								<th>제목</th>
								<td><input type="text" name="evn_subject" id="evn_subject" class="frm_input" value="${info.evn_subject}"/></td>
							</tr>
			...
			<div class="btn_area textcenter">	
				...
				<c:if test="${info.evn_trash eq '1' }">
					<a href="/admin/sosig/aevent/trash.do" class="btn_size2 btn_bordergray">취소</a>
				</c:if>
				
				
				<!-- 일반게시판에서 글 조회,수정 -->
				<c:if test="${info.evn_trash eq '0' && info.evn_no ne null}">
					<a href="/admin/sosig/aevent/list.do" class="btn_size2 btn_bordergray">취소</a>
...
function eco_del(){
  ...
  if($("#evn_content").val() == ""){
    ...
    if(document.getElementById("evn_content").value==' <p>&nbsp;</p>'){
     ...
     try {
    	 if($('#evn_no').val()!=''){
    		 $("#imform").attr("action","/admin/sosig/event/update_data.do");
				$("#imform").submit(); 
			}else{
				$("#imform").attr("action","/admin/sosig/event/insert_data.do");
