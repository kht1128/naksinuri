...
<script charset="UTF-8" src="http://s1.daumcdn.net/svc/attach/U03/cssjs/mapapi/libs/1.0.1/1515130215283/services.js"></script>
...
<div>
	<form:form commandName="imform" id="imform" method="post" enctype="multipart/form-data" >
		...
		<input type="hidden" name="returnUrl" value="<c:url value='/share/usage/updateview.do'/>"/>
	...
	<div id="customerSound" class="content respon3">
		<section id="writeForm" class="write_box">
			...
			<dl>
				...
				<dd><textarea class="write_textarea" rows="10" id="bo_content" name="bo_content" style="width:98%" title="낚시용품 사용기 내용">${info.bo_content}</textarea></dd>
			</dl>
			<dl>
				<dt>파일첨부</dt>
				<dd>
					...
					<div id="btnArea">
						...
						<td><a href="#;" class="btn_white" onclick="insertinput()" title="첨부파일 추가">첨부파일 추가 </a></td>
...
function submitContents() {
	...
	/* 2019.09.23 개인정보보호를 위해 이메일,연락처 수집 제거
...
if ($("#bo_content").val() == " <p>&nbsp;</p>") {
	...
	try {
		...
		if ($('#bo_sn').val() != '') {
				$("#imform")
						.attr("action", "/share/usage/update_data.do");
				$("#imform").submit();
			} else {
				$("#imform")
						.attr("action", "/share/usage/insert_data.do");
				document.getElementById("imform").submit();
			}
