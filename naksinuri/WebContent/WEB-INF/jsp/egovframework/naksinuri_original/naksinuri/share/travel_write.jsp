...
<div>
	<form:form commandName="imform" id="imform" method="post" enctype="multipart/form-data" >
		...
		<input type="hidden" name="returnUrl" value="<c:url value='/share/travel/updateview.do'/>"/>
	...
	<div id="customerSound" class="content respon3">
		<section id="writeForm" class="write_box">
			...
			<dl>
				<dt>여행 카테고리</dt>
				<dd>
					<select class="naksi_select" id="bo_cate" name="bo_cate" value="${info.bo_cate}" title="여행카테고리 옵션선택(바다,민물)">
			...
			<dl>
				<dt>이메일</dt>
				<dd>
				<input type="text" size="30" class="write_input" placeholder="이메일을 입력하세요" name="bo_email1" id="bo_email1" value="${info.bo_email1 }">@
			...
			<dl>
				...
				<dd>
					<label for="bo_content" class="blind">내용입력</label>
					<textarea class="write_textarea" rows="10" id="bo_content" name="bo_content" style="width:98%" title="낚시여행기 내용입력">${info.bo_content}</textarea>
				</dd>
			</dl>
			<dl>
				<dt>파일첨부</dt>
				<dd>
					...
					<div id="btnArea">
						...
						<td><a href="javascript:void(0);" class="btn_white" onclick="insertinput()" tabindex="0" onkeyPress="if (event.keyCode==13 || event.keyCode==32){insertinput();}" title="첨부파일 추가">첨부파일 추가 </a></td>

						<td><a href="javasript:void(0);" class="btn_gray" onclick="deleteinput()" tabindex="0" onkeyPress="if (event.keyCode==13 || event.keyCode==32){deleteinput();}" title="파일 삭제">파일 삭제</a></td>
					</div>
...
// 	 email1 = $("#bo_email1").val();
// 	 email2 = $("#bo_email2").val();\
...
if ($("#bo_content").val() == " <p>&nbsp;</p>") {
	...
	try {
		...
		if ($('#bo_sn').val() != '') {
				$("#imform")
						.attr("action", "/share/travel/update_data.do");
				$("#imform").submit();
			} else {
				$("#imform")
						.attr("action", "/share/travel/insert_data.do");
				document.getElementById("imform").submit();
			}
