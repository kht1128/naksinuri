	...
	<div id="survey" class="content respon2">
		<section id="suveyPage">
			<div class="tit">
				...
				<span class="date" readonly>설문기간 : <b>${info.sv_strt_dt } ~ ${info.sv_end_dt }</b></span>
			</div>
			<div class="suveybox">
<%-- 				<span class="subject" readonly>제목 : <b>${info.sv_subject }</b></span> --%>
<%-- 				<span class="name" readonly>작성자 : <b>${info.bo_name }</b></span> --%>
				<c:forEach var="item" items="${quest}" varStatus="status">
					<dl id="div_sv_qst${item.sq_id}" class="div_sv_qst">
						<dt><em><fmt:formatNumber var="no" minIntegerDigits="2" value="${item.svq_num}" type="number"/>${no}.</em><span>${item.svq_subject} <c:if test="${item.svq_mxcnt gt 1 }">[중복 선택 가능]	${item.svq_mxcnt}개 이하<br></c:if></span></dt>
						<dd>
			...
			<div id="btnArea">
				<ul class="floats">
					<a href="#;" class="btn_request btn_blue"  onclick="submitContents('등록되었습니다!');">제출</a>
					<a href="./list.do" class="btn_prev btn_white">취소</a></li>
				</ul>
			</div>
		</section>
	</div>
...
function chgaddr(){
	...
	function submitContents(){
		...
		$("#imform").attr("action","/survey/survey/m/answer.do");
		$("#imform").submit(); 
	}
