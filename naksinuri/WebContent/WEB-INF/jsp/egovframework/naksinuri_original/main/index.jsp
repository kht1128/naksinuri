...
.info.type1 .title {
	background-color: #fe4c27 !important;
}
...
.info.type4 .title {
	background-color: #60e640 !important;
}
...
.info .body dl:nth-child(2n) {
	background-color: #f3f3f3;
}

.info .body dl dt, .info .body dl dd {
	width: 50%;
	text-align: center;
}
...
<form:form commandName="viewform" id="viewform" method="post"
		...
		<div class="wrap_visual">
			<div class="visual-slide">
				<div class="control-box">
					<!-- 페이징 -->
					<div
						class="swiper-pagination pagination_progress swiper-pagination-progressbar">
					...
					<div
						class="swiper-pagination pagination_fraction swiper-pagination-fraction">
						<span class="swiper-pagination-current">4</span> / <span
							class="swiper-pagination-total">4</span>
					</div>

					<!--김현태추가 재생 정지 버튼 -->
					<div class="swiper-button-prev swiper-button-prev_s"
						title="배경 이미지 이전 슬라이드" tabindex="0" role="button"
						aria-label="Previous slide" aria-disabled="false">배경 이미지 이전
						슬라이드</div>
					<div class="controler">
						<button class="sponsor_autoplay-control" title="배경 이미지 슬라이드 정지,재생"></button>
					</div>
					<div
						class="swiper-button-next swiper-button-next_s swiper-button-disabled"
						title="배경 이미지 다음 슬라이드" tabindex="0" role="button"
						aria-label="Next slide" aria-disabled="true">배경 이미지 다음 슬라이드</div>
				</div>
				...
				<div class="swiper-container">
					<div class="swiper-wrapper">
						<div class="swiper-slide">
							<img class="sum"
								src="/naksinuri_original/common_main/img/main/new_main01_bg.png"
								alt="">
						</div>
						<div class="swiper-slide">
							<img class="sum"
								src="/naksinuri_original/common_main/img/main/new_main02_bg.png"
								alt="">
						</div>
						<div class="swiper-slide">
							<img class="sum"
								src="/naksinuri_original/common_main/img/main/new_sec01_bg.png"
								alt="">
						</div>
						<div class="swiper-slide">
							<img class="sum"
								src="/naksinuri_original/common_main/img/main/new_main03_bg.png"
								alt="">
						</div>
					</div>
			...
			<div class="shortcut_list">
				<ul>
					<li><a href="/educenter/trnng/list.do" title="교육일정 바로가기">
							<span class="shortcut_txt ico1">교육일정</span>
					</a></li>
					<li><a href="/educenter/main/serviceGuide.do"
						title="교육안내 바로가기"> <span class="shortcut_txt ico2">교육안내</span>
					</a></li>
					<li><a href="/educenter/board/faq/list.do" title="수강 FAQ 바로가기">
		...
		<div class="wrap">
			...
			<div class="main_sec05_container">
				<article class="left_cont">
					<ul>
						<li><img
							src="../naksinuri_original/common_main/img/main/isu.png" alt="">
							<h4>이수증 내역</h4> <a href="/educenter/mbrhstry/list.do"
							title="이수증 내역 바로가기">GO</a>
							<p>
								이수증 내역에서<br>열람이 가능합니다.
							</p></li>
						<li><img
							src="../naksinuri_original/common_main/img/main/edu.png" alt="">
							<h4>온라인 교육신청</h4> <a href="/educenter/trnng/list.do"
							title="온라인 교육 신청 바로가기">GO</a>
							<p>
								신청하기 버튼이<br>활성화 되어 있는 교육은<br>신청이 가능합니다.
							</p></li>
						<li><img
							src="../naksinuri_original/common_main/img/main/youtb.png" alt="">
							<h4>낚시의 품격</h4> <a href="/lesson/dignity/list.do"
							title="낚시의 품격 바로가기">GO</a>
							<p>
								실전 낚시의 품격에서<br>ATOZ를 확인해보세요.
							</p></li>
					</ul>
				</article>
				<!-- 안전해 -->
				...
				<div class="left_cont">
					...
					<div class="top_cont">
						...
						<c:forEach var="item" varStatus="status" items="${list_crs}">
							<fmt:parseDate value="${fn:replace(item.CRS_STR_DT, '.0', '')}"
								var="parse_crs_str_dt" pattern="yyyy-MM-dd" scope="page" />
							<fmt:parseDate value="${fn:replace(item.CRS_END_DT, '.0', '')}"
								var="parse_crs_end_dt" pattern="yyyy-MM-dd" scope="page" />
							<fmt:parseDate
								value="${fn:replace(item.RECRUIT_STR_DT, '.0', '')}"
								var="parse_recruit_str_dt" pattern="yyyy-MM-dd" scope="page" />
							<fmt:parseDate
								value="${fn:replace(item.RECRUIT_END_DT, '.0', '')}"
								var="parse_recruit_end_dt" pattern="yyyy-MM-dd" scope="page" />
							<fmt:formatDate value="${parse_crs_str_dt}"
								pattern="yyyy.MM.dd (E)" var="CRS_STR_DT" />
							<fmt:formatDate value="${parse_crs_end_dt}"
								pattern="yyyy.MM.dd (E)" var="CRS_END_DT" />
							<fmt:formatDate value="${parse_crs_str_dt}" pattern="yyyy.MM.dd"
								var="CRS_ym" />
							<fmt:formatDate value="${parse_crs_str_dt}" pattern="MM.dd"
								var="CRS_day" />
							<fmt:formatDate value="${parse_recruit_str_dt}"
								pattern="yyyy.MM.dd (E)" var="RECRUIT_STR_DT" />
							<fmt:formatDate value="${parse_recruit_end_dt}"
								pattern="yyyy.MM.dd (E)" var="RECRUIT_END_DT" />
							...
							<div class="edu_cont">
								<c:choose>
									<c:when
										test="${item.TYPE_GB eq 'offline' and item.CRS_MBR_CD eq 'CIDN010300'}">
										<c:if test="${found1 eq false}">
											<a href="/educenter/trnngoffline/fshhpList.do"
												class="edu_list" title="${item.CRS_NM}">
												<div class="prg1">
												...
												<div class="prg2">
													...
													<div class="sub_text_inner">
														<strong>교육장소</strong><span class="location_inner">${item.CRS_ADDR}</span>
													</div>
													<div class="sub_text_inner">
														<strong>신청인원</strong><span class="people_inner">${item.MBR_CUR_CNT}/${item.MBR_MAX_CNT}명</span>
													</div>
												</div> <c:choose>
													<c:when
														test="${item.MBR_CUR_CNT eq '20' or RECRUIT_END_DT < sysYear}">
														<div class="prg3">
															<p class="whether_box">
																<img
																	src="/naksinuri_original/common_main/img/main/whether_boxn_01.png"
																	alt="">
															</p>
														</div>
													</c:when>
													<c:otherwise>
														<div class="prg3">
															<p class="whether_box">
																<img
																	src="/naksinuri_original/common_main/img/main/whether_box_01.png"
																	alt="">
															</p>
														</div>
													</c:otherwise>
								...
								<c:choose>
									<c:when
										test="${item.TYPE_GB eq 'offline' and item.CRS_MBR_CD eq 'CIDN010200'}">
										<c:if test="${found2 eq false}">
											<a href="/educenter/trnngoffline/fshlcList.do"
												class="edu_list" title="${item.CRS_NM}">
												<div class="prg1">
													<h5 class="point_inner">${CRS_day}</h5>
													<span class="date_inner">${CRS_ym}</span>
												...
												<div class="prg2">
													...
													<c:when
														...
														<div class="prg3">
															<p class="whether_box">
																<img
																	src="/naksinuri_original/common_main/img/main/whether_boxn_02.png"
																	alt="">
															</p>
														</div>
													</c:when>
													<c:otherwise>
														<div class="prg3">
															<p class="whether_box">
																<img
																	src="/naksinuri_original/common_main/img/main/whether_box_02.png"
																	alt="">
															</p>
														</div>
													</c:otherwise>
										...
										<%-- <c:if test="${found2 eq false}">
											<div class="edu_list">
												...
												<div class="prg2">
													...
													<div class="sub_text_inner">
														<strong>교육장소</strong><span class="location_inner">-</span>
													</div>
													<div class="sub_text_inner">
														<strong>신청인원</strong><span class="people_inner">-</span>
													</div>
												</div>
												<div class="prg3">
													<p class="whether_box">
							...
							<div class="edu_list_inner">
								...
								<c:forEach var="item" varStatus="status" items="${list_crs}">
									...
									<c:if test="${not found3}">
										<c:choose>
											<c:when
												test="${item.TYPE_GB eq 'online' and item.CRS_MBR_CD eq 'CIDN010200'}">

												<a href="/educenter/trnng/list.do" class="edu_list2"
													title="${item.CRS_NM}">
													<div>
														<p class="text_inner">${item.CRS_NM}</p>
														<div class="sub_text_inner">
															<strong>교육일</strong><span class="location_inner">${RECRUIT_STR_DT}~${RECRUIT_END_DT}</span>
														</div>
														<div class="sub_text_inner">
															<strong>신청인원</strong><span class="people_inner">${item.MBR_CUR_CNT}
																/ 무제한</span>
														</div>
													</div>
													<div>
											...
											<c:otherwise>
												...
												<%-- <div class="edu_list2">
													<div>
														...
														<div class="sub_text_inner">
															<strong>교육일</strong><span class="location_inner">-</span>
														</div>
														<div class="sub_text_inner">
															<strong>신청인원</strong><span class="people_inner">-</span>
														</div>
									...
									<c:if test="${not found4}">
										<c:choose>
											<c:when
												test="${item.TYPE_GB eq 'online' and item.CRS_MBR_CD eq 'CIDN010300'}">
				...
				<div class="right_cont">
					...
					<div class="info_madang">
						<div class="info_madang_title_inner">
							<h5 style="color: #757575;" class="info_madang_title">알림 게시글을 확인하세요</h5>
							<a href="/promotion/info/list.do" class="info_madang_add_btn"
								title="알림마당 바로가기">더보기</a>
						</div>
						...
						<div class="info_madang_cont">
							<ul>
								<c:forEach var="item" varStatus="status" items="${info_list }">
									<li class="info_madang_list"><a href="javascript:void(0);"
										class="tit" onclick="infoview('${item.bo_sn}')"
										title="알림마당  ${item.bo_subject } 페이지이동"><p>${item.bo_subject}</p>
											<span class=""> ${fn:substring(item.bo_insert_dt,0,10) }</span></a>
									</li>
								...
								<ul>
									<c:forEach var="item" varStatus="status"
										...
										<li><a href="javascript:void(0);"
											onclick="noticeview('${item.bo_sn}')"
											title="공지사항  ${item.bo_subject } 페이지이동"><p
													class="notice_text dot-list2">${item.bo_subject}</p>
												<span class="notice_date">
													${fn:substring(item.bo_insert_dt,2,10) }</span></a></li>
									</c:forEach>
								</ul>
							</div>
							<div class="notice_list_cont notice_cont2">
								<ul>
									<c:forEach var="item" varStatus="status" items="${info_list2 }">
										<li><a href="javascript:void(0);"
											onclick="infoview('${item.bo_sn}')"
											title="알림마당  ${item.bo_subject } 페이지이동"><p
													class="notice_text dot-list2">${item.bo_subject}</p>
												<span class="notice_date">
													${fn:substring(item.bo_insert_dt,2,10) }</span></a></li>
									</c:forEach>
							...
							<div class="notice_list_cont notice_cont3">
								<ul>
									<c:forEach var="item" varStatus="status"
										items="${policy_list2 }">
										<li><a href="javascript:void(0);"
											onclick="policyview('${item.bo_sn}')"
											title="낚시정책  ${item.bo_subject } 페이지이동"><p
													class="notice_text dot-list2">${item.bo_subject}</p>
												<span class="notice_date">
													${fn:substring(item.bo_insert_dt,2,10) }</span></a></li>
									</c:forEach>
								...
								<ul>
									<c:choose>
										<c:when test="${empty campaign_list}">
											<li><a><p class="notice_text dot-list2">진행중인
														캠페인이 없습니다.</p>
													<span class="notice_date">-</span></a></li>
										</c:when>
										<c:otherwise>
											<c:forEach var="item" varStatus="status"
												items="${campaign_list}">
												<li><a href="javascript:void(0);"
													onclick="contsdesignview('${item.bo_sn}')"
													title="낚시캠페인 ${item.bo_subject} 페이지이동"><p
															class="notice_text dot-list2">${item.bo_subject}</p>
														<span class="notice_date">${fn:substring(item.bo_insert_dt,2,10)}</span></a></li>
											</c:forEach>
										</c:otherwise>
									</c:choose>
								</ul>
				...
				<div class="dignity-slide">
					<div class="con_area">
								...
								<c:forEach var="item" varStatus="status"
									...
									<div class="swiper-slide">
										<a href="javascript:void(0);"
											onclick="dignityview('${item.bo_sn}')"
											title="낚시의 품격  ${item.bo_subject } 페이지이동">
											<div class="thumb">
												<img class="type2"
													src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${item.bo_main_img}"/>&fileSn=<c:out value="${item.file_sn }"/>'
													alt="${item.bo_subject}" />
											</div>
											<div class="txt_area">
						...
						<div class="control-box anothertype">
							<!-- 페이징 -->
							<div
								class="swiper-pagination swiper-pagination02 swiper-pagination-fraction">
								<span class="swiper-pagination-current">3</span> / <span
									class="swiper-pagination-total">3</span>
							</div>
							<!-- 이전 버튼 -->
							<div class="swiper-button-prev swiper-button-prev02"
								title="낚시의 품격 이전 슬라이드" role="button" aria-label="Previous slide">낚시의
								품격 이전 슬라이드</div>
							<!-- 재생 정지 버튼 -->
							...
							<!-- 다음 버튼  -->
							<div class="swiper-button-next swiper-button-next02"
								title="낚시의 품격 다음 슬라이드" role="button" aria-label="Next slide">낚시의
								품격 다음 슬라이드</div>
						</div>
			...
			<article class="con con03">
				...
				<div class="dignity-slide2">
					...
					<div class="con_area" style="overflow: hidden;">
						<div class="swiper-container swiper-container-horizontal">
							<div class="swiper-wrapper"
								style="transform: translate3d(-1840px, 0px, 0px); transition-duration: 0ms;">
								...
								<div class="swiper-slide swiper-slide-duplicate-active"
									...
									<div>
										<div class="thumb">
											<img class="type2"
												src="/naksinuri_original/common_main/img/main/add_img01.png"
								...
								<div class="swiper-slide" data-swiper-slide-index="">
									<div>
										<div class="thumb">
											<img class="type2"
												src="/naksinuri_original/common_main/img/main/add_img02.png"
												alt="" style="height: 350px;">
										</div>
										<div class="txt_area">
											<p class="tit"></p>
								...
								<div
									...
									<div>
										<div class="thumb">
											<img class="type2"
												src="/naksinuri_original/common_main/img/main/add_img03.png"
												alt="" style="height: 350px;">
										</div>
										<div class="txt_area">
											<p class="tit"></p>
								...
								<div
									...
									<div>
										<div class="thumb">
											<img class="type2"
												src="/naksinuri_original/common_main/img/main/add_img04.png"
												alt="" style="height: 350px;">
										</div>
										<div class="txt_area">
											<p class="tit"></p>
								...
								<div
									...
									<div>
										<div class="thumb">
											<img class="type2"
												src="/naksinuri_original/common_main/img/main/add_img05.jpeg"
												alt="" style="height: 350px;">
										</div>
										<div class="txt_area">
											<p class="tit"></p>
								...
								<div
									...
									<div>
										<div class="thumb">
											<img class="type2"
												src="/naksinuri_original/common_main/img/main/add_img06.png"
												alt="" style="height: 350px;">
										</div>
										<div class="txt_area">
											<p class="tit"></p>
						...
						<div class="control-box anothertype">
							<!-- 페이징 -->
							<div
								class="swiper-pagination swiper-pagination04 swiper-pagination-fraction">
								<span class="swiper-pagination-current">3</span> / <span
									class="swiper-pagination-total">3</span>
							</div>
							<!-- 이전 버튼 -->
							<div class="swiper-button-prev swiper-button-prev04"
								title="카드뉴스 이전 슬라이드" role="button" aria-label="Previous slide">카드뉴스
								이전 슬라이드</div>
							<!-- 재생 정지 버튼 -->
							...
							<!-- 다음 버튼  -->
							<div class="swiper-button-next swiper-button-next04"
								title="카드뉴스 다음 슬라이드" role="button" aria-label="Next slide">카드뉴스
								다음 슬라이드</div>
						</div>
					</div>
				</div>
		...
		<div class="wrap">
			...
			<article class="con con01">
				...
				<div class="con_area">
					<div class="info_list">
						<ul>
							<li><a href="http://www.khoa.go.kr/swtc/main.do"
								target="_blank" title="조석정보 외부링크"> <img
									src="/naksinuri_original/common_main/img/main/info_ico_1.png"
									class="info_img" alt="">
							</a></li>
							<li><a
								href="http://www.khoa.go.kr/kcom/cnt/selectContentsPage.do?cntId=31201000"
								target="_blank" title="바다갈라짐 외부링크"> <img
									src="/naksinuri_original/common_main/img/main/info_ico_2.png"
									class="info_img" alt="">
							</a></li>
							<li><a
								href="http://www.khoa.go.kr/koofs/kor/observation/obs_real.do?obsItem=WT_TEM&amp;imgIdx=05"
								target="_blank" title="수온정보 외부링크"> <img
									src="/naksinuri_original/common_main/img/main/info_ico_3.png"
									class="info_img" alt="">
							</a></li>
							<li><a href="http://www.kma.go.kr/index.jsp" target="_blank"
								title="날씨정보 외부링크"> <img
									src="/naksinuri_original/common_main/img/main/info_ico_4.png"
									class="info_img" alt="">
							</a></li>
							<li><a
								href="http://www.khoa.go.kr/koofs/kor/seawf/sea_wf.do?menuNo=01&amp;link="
								target="_blank" title="조류예보 외부링크"> <img
									src="/naksinuri_original/common_main/img/main/info_ico_5.png"
									class="info_img" alt="">
							</a></li>
							<li><a href="http://www.khoa.go.kr/Onbada/main.do"
								target="_blank" title="해양예보방송 외부링크"> <img
									src="/naksinuri_original/common_main/img/main/info_ico_6.png"
									class="info_img" alt="">
							</a></li>
						</ul>
					</div>
			...
			<article class="con con03">
				...
				<div class="con_area">
					<div class="counseling">
						<img src="/naksinuri_original/common_main/img/call_char.png"
							alt=""> <a href="tel:1833-7139" class="call">1833-7139</a>
						<p class="edu_hour">
							<strong class="edu_str">평일</strong> 오전 09:00 ~ 11:30
						</p>
						<p class="edu_hour">
							<strong class="edu_str">평일</strong> 오후 13:00 ~ 17:30
						</p>
					</div>
					<!-- <div class="counseling">
		...
		<div class="inner">
			<div class="sponsor-slide">
				<div class="control_btn">
					<!-- 이전 버튼 -->
					<div
						class="swiper-button-prev swiper-button-prev03 sponsor_btn_prev"
						title="스폰서 이전 슬라이드" tabindex="0" role="button"
						aria-label="Previous slide">스폰서 이전 슬라이드</div>
					...
					<!-- 다음 버튼  -->
					<div
						class="swiper-button-next swiper-button-next03 sponsor_btn_next"
						title="스폰서 다음 슬라이드" tabindex="0" role="button"
						aria-label="Next slide">스폰서 다음 슬라이드</div>
				</div>
				<div class="swiper-container swiper-container-horizontal">
					<div class="swiper-wrapper"
						style="transition-duration: 0ms; transform: translate3d(-3432.8px, 0px, 0px);">
						...
						<div class="swiper-slide">
							<a href="http://www.mof.go.kr/" target="_blank">해양수산부</a>
						</div>
						<div class="swiper-slide">
							<a href="https://www.fipa.or.kr/" target="_blank">FIPA
								한국어촌어항공단</a>
						</div>
						<div class="swiper-slide">
							<a href="https://www.facebook.com/FIPAkor" target="_blank">한국어촌어항공단<br>공식
								페이스북 페이지
							</a>
						</div>
						<div class="swiper-slide">
							<a href="http://www.sealife.go.kr" target="_blank">BLUE OCEAN
								GUIDE<br>귀어귀촌 종합센터
							</a>
						</div>
						<div class="swiper-slide">
							<a href="http://www.kff.or.kr/" target="_blank">KFF 한국낚시연합</a>
						</div>
						<div class="swiper-slide">
							<a href="http://www.nifs.go.kr/" target="_blank">국립수산과학원</a>
						</div>
						<div class="swiper-slide">
							<a href="http://www.fira.or.kr/" target="_blank">FIRA
								한국수산자원관리공단</a>
						</div>
						<div class="swiper-slide">
							<a href="http://www.kst.or.kr/" target="_blank">선박안전기술공단</a>
						</div>
						<div class="swiper-slide">
							<a href="https://www.komsa.or.kr/" target="_blank">KOSMA<br>한국해양교통안전공단
							</a>
						</div>
						<div class="swiper-slide">
							<a href="http://www.khoa.go.kr/" target="_blank">국립해양조사원
								KHOA</a>
						</div>
						<div class="swiper-slide">
							<a href="http://www.koreafca.kr/" target="_blank">사단법인
								한국낚시업중앙회</a>
						</div>
...
<script>
	$(function() {
		...
		// 낚시의 품격 슬라이드
		var dignitySlide = new Swiper(
				'.dignity-slide .swiper-container',
				{
					slidesPerView : 1,
					loop : true,
					autoplay : {
...
<form:form commandName="eduCenterBoardVO" id="noticeEduForm"
	name="noticeEduForm" method="post">
	<input type="hidden" id="BD_SN" name="BD_SN" value="" />
</form:form>
<form:form commandName="eduMyHistoryVO" id="crsViewForm"
	name="crsViewForm" method="post">
...
<script>
	...
	const
	selectBoxElements = document.querySelectorAll(".custom-select");
	...
	function selectOption(optionElement) {
		const
		selectBox = optionElement.closest(".custom-select");
		const
		selectedElement = selectBox.querySelector(".selected-value");
	...
	function handleEnterKey(e) {
		if (e.key === "Enter" || e.keyCode === 13) {
			...
			const
			isOptionElement = targetElement.classList.contains("option");
	...
	document.addEventListener("click", function(e) {
		...
		targetElement = e.target;
		const
		isSelect = targetElement.classList.contains("custom-select")
				|| targetElement.closest(".custom-select");
		...
		const
		allSelectBoxElements = document.querySelectorAll(".custom-select");
...
<script>
	const
	arrOverlayObjContainer = {};//맵 배치 팝업 모음
	let
	clusterer;
	const
	...
	function fnSelectInfs(test) {
		...
		const
		form = document.getElementById('supform');
		if (test == 0)
		...
		$
				.ajax({
					...
					beforeSend : function() {
						...
						//화면에 출력할 마스크를 설정해줍니다.
						const
						mask = "<div id='mask' style='position:absolute; z-index:199; background-color:#fff; display:none; left:0; top:0;'></div>";
						let
						loadingImg = '';
						...
						//화면에 레이어 추가
						$('#map').append(mask).append(loadingImg)
					...
					success : function(data, status, xhr) {
						...
						listRawData = [];
						const
						oceanFshIdex_list = JSON.parse(data.oceanFshIdex_list);
						for (var i = 0; i < oceanFshIdex_list.length; i++) {
							...
							obj['addrSeq'] = oceanFshIdex_list[i].name + '_'
									+ i;
...
<script>
				...
				markers = $(item)
						.map(
								function(i, item) {
									...
									const
									position = new kakao.maps.LatLng(item.lat,
											item.lon);
									// 마커 이미지의 이미지 크기 입니다
									const
									...
									//console.log(marker);
									/* $(marker.ca).parent().addClass('focus_add')
											.attr('tabindex', '0').css(
													'height', '40px').css(
													'width', '25px');
									$(marker.ca).parent().attr('id', item.name); //포커싱
									$(marker.ca).parent().keyup(
											function(e) {
												if (e.keyCode == 13) {
													$('.focus_add').attr(
															'tabindex', '-1');
									...
									kakao.maps.event
											.addListener(
													...
													function() {
														...
														Object
																.keys(
																		arrOverlayObjContainer)
																.forEach(
																		function(
																				k) {
																			if (k != item.addrSeq)
																				arrOverlayObjContainer[k]
																						.setMap(null);
																		});
	...
	function makeOverlay(targetObj) {
		...
		const
		markerPosition = new kakao.maps.LatLng(mapX, mapY);
		let
		imageSrc = '';// 마커이미지의 주소    
		const
		...
		$
				.ajax({
					...
					success : function(data, status, xhr) {
						const
						airTemp = data.info.air_temp;
						const
						waterTemp = data.info.water_temp;
						const
						waveHeight = data.info.wave_height;
						let
						type;
						...
						const
						content = '<div class="wrap ' + wrapStyle + '" id="' + addrSeq + '">'
								+ '		<div class="info ' + type +'">'
								+ '        <div class="title">'
								+ '            <div tabindex="0" class="close" onkeydown="if(event.keyCode === 13) { closeOverlay(\''
								+ addrSeq
								+ '\'); }" onclick="closeOverlay(\''
								+ addrSeq
								+ '\')" title="닫기"><span aria-hidden="true">×</span></div>'
								+ '        	 <div class="top">'
								...
								+ '            	<span class="date" id="currentDate">'
								+ date
								+ '</span>'
								+ '        	 </div>'
								+ '			 <span class="area" id="currentName">'
								+ name
								+ '</span>'
								+ '        </div>'
								+ '        <div class="body">'
