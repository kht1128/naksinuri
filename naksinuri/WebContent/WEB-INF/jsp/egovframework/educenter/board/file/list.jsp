				...
				<tbody>
					<c:forEach var="item" items="${list_fixed}" varStatus="status">
						<tr class="noti">
							...
							<td class="file alignCenter mbNone"><a href="#;" class="colorBlue"><c:if test="${not empty item.BD_FILE}"><i class="fas fa-file-alt"></c:if></i></a></td>
					...
					<c:forEach var="item" items="${list}" varStatus="status">
						<tr class="noti">
							...
							<td class="file alignCenter mbNone">
								<c:if test="${not empty item.BD_FILE}">
									<img alt="첨부파일" src="/naksinuri_original/common_main/img/icon_file.gif">
								</c:if>
							</td>
							<td class="writer alignCenter mbNone"><c:out value="${item.MBR_NCNM}"/></td>
							<td class="hit alignCenter mbNone"><c:out value="${item.BD_VIEW_CNT}"/></td>
...
function fnView(bdsn) {
    ...
    document.viewForm.action = "<c:url value='/educenter/board/file/view.do'/>";
    document.viewForm.submit();
}

function fnLinkPage(pageNo){
    document.listForm.pageIndex.value = pageNo;
    document.listForm.action = "<c:url value='/educenter/board/file/list.do'/>";
