...
<form:form commandName="eduMyHistoryVO" id="listForm2" name="listForm2"
	method="post">
	<input type="hidden" id="CRS_SN" name="CRS_SN" value="" />
	<input type="hidden" id="FOCUS_RETURN" name="FOCUS_RETURN" value="" />
	<input type="hidden" id="FOCUS_RETURN2" name="FOCUS_RETURN2" value="" />
</form:form>
...
<ul class="grpedu_ul">
	<li><ul class="grpedu_ul_1">
			...
			<c:if test="${empty list_default_1}">
				<li>
					<div class="grpedu_Pn">
						<span class="tag">낚시어선업자 및 선원</span> <a href="#;">
			...
			<c:forEach var="item" varStatus="status" items="${list_default_1}">
				...
				<fmt:parseDate value="${fn:replace(item.RECRUIT_STR_DT, '.0', '')}"
					var="parse_recruit_str_dt" pattern="yyyy-mm-dd" scope="page" />
				<fmt:parseDate value="${fn:replace(item.RECRUIT_END_DT, '.0', '')}"
					var="parse_recruit_end_dt" pattern="yyyy-mm-dd" scope="page" />
				<fmt:formatDate value="${parse_crs_str_dt}" pattern="yyyy.mm.dd"
					var="CRS_STR_DT" />
				<fmt:formatDate value="${parse_crs_end_dt}" pattern="yyyy.mm.dd"
					var="CRS_END_DT" />
				<fmt:formatDate value="${parse_recruit_str_dt}" pattern="yyyy.mm.dd"
					var="RECRUIT_STR_DT" />
				<fmt:formatDate value="${parse_recruit_end_dt}" pattern="yyyy.mm.dd"
					var="RECRUIT_END_DT" />
				<li>
					<div class="grpedu_Pn">
						<div class="tag">낚시어선업자 및 선원</div>
						<div id="${status.index}" class="clk_modal_eduinfo"
							data-crs-sn="${item.CRS_SN}"
							data-linkurl="/educenter/trnng/view.do"
							onkeyPress="if (event.keyCode==13 || event.keyCode==32){$('.clk_modal_eduinfo').click();}"
							title="${item.CRS_PLACE} 상세보기">
							<p class="dates">${CRS_STR_DT}&nbsp;~&nbsp;${CRS_END_DT}</p>
							<a href="javascript:void(0);" class="sbj"
								title="${item.CRS_PLACE}" data-crs-sn="${item.CRS_SN}"
								data-linkurl="/educenter/trnng/view.do"
								id="focusLine${status.index}">${item.CRS_PLACE}</a>
							<div class="cont">${item.CRS_ADDR}</div>
						</div>
					</div>
				</li>
			</c:forEach>
	...
	<li><ul class="grpedu_ul_2">
			...
			<c:if test="${empty list_default_2}">
				<li>
					<div class="grpedu_Pn">
						<span class="tag">낚시터업자</span> <a href="#;">
							<p class="dates">준비된 교육이 없습니다.</p>
						</a>
					</div>
				</li>
			...
			<c:forEach var="item" varStatus="status" items="${list_default_2}">
				...
				<li>
					<div class="grpedu_Pn">
						...
						<div id="${status.index}" class="clk_modal_eduinfo" data-crs-sn="${item.CRS_SN}"
							data-linkurl="/educenter/trnng/view.do"
							onkeyPress="if (event.keyCode==13 || event.keyCode==32){$('.clk_modal_eduinfo').click();}"
							title="${item.CRS_PLACE} 상세보기">
							<p class="dates">${CRS_STR_DT}&nbsp;~&nbsp;${CRS_END_DT}</p>
							<a href="javascript:void(0)" title="${item.CRS_PLACE}"
								class="sbj" data-crs-sn="${item.CRS_SN}"
								data-linkurl="/educenter/trnng/view.do"
								id="focusLine${status.index}">${item.CRS_PLACE}</a>
							<div class="cont">${item.CRS_ADDR}</div>
						</div>
...
/* 	$(".close").click(function() {
		var id_check = 	$("#FOCUS_RETURN").val();
		alert(id_check)
	
		$(".eduinfo_Bx").focus();
	}); */
	$('#allPublicModal').on('hidden.bs.modal', function (e) {
			var id_check =$("#FOCUS_RETURN").val();

			$("#focusLine"+id_check).focus();
		})	
