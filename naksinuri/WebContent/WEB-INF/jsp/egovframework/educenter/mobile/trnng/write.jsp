...
<style>
	...
	.emphasis {color: #B75552;font-size: 1.6rem;}
	.emphasis2 {color: #fff;font-size: 1.7rem;background-color: #C00000;line-height: 4.3rem;padding: 6px;}
...
<form:form commandName="myHistoryVO" id="listForm" name="listForm" method="post">
	...
	<div class="content respon3">
									...
									<c:when test="${info_crs.CRS_MBR_CD eq item.CD_ID}">
									  <c:choose>
										...
										<c:when test="${item.CD_NM eq '낚시어선업자' and info2.TYPE_GB eq 'offline'  and info2.CRS_LAW_TYPE ne 'default'}">
										  ${item.CD_NM}<br>「낚시관리법」 제25조(낚시어선업의 신고)에 따른 낚시어선업을 하려는 자, <br>제38조(영업의 폐쇄 등)제1항제5호(안전사고 발생)에 해당하여 영업정지명령을 받은 후 영업을 재개하려는 자
										</c:when>
										<c:otherwise>
										  <!-- 다른 조건이 있을 경우 처리 -->
										</c:otherwise>
		...
		<section id="writeForm" class="write_box mt-30">
			<h2 class="font-size-18">${CRS_MBR_NM } 전문교육 수강신청서</h2>
			<div class="agree_box">
				<h3 class="mt-10">기본회원정보</h3>
				<table class="basic_tbl mt-0">
					...
					<tbody>		
						<tr>
							...
							<td><input type="text" class="basic_input w100 readonly" name="MBR_NM" value="${info_mbr.MBR_NM}" disabled></td>
						</tr>
						<tr>
							<th class="text-center">생년월일</th>
							<td><input type="text" class="basic_input w100 readonly" value="${info_mbr.MBR_BIRTH}" disabled></td>
						</tr>
						<tr>
							<th class="text-center">주 소</th>
							<td ><input type="text" class="basic_input w100 readonly" value="${info_mbr.MBR_ADDR1} ${info_mbr.MBR_ADDR2}" disabled></td>
						</tr>
						<tr>
							<th class="text-center">연 락 처</th>
							<td ><input type="text" name="MBR_HP" class="basic_input w100 readonly" value="${info_mbr.MBR_HP}" disabled></td>
						</tr>						
					</tbody>
				</table>
							...
							<tbody>
								<tr>
									...
									<td>
										...
										<c:if test="${row.DTL_LICENSE_CD eq 'CIDL00002'}"><p class="mt-5"><b>*교육책임자 지정 확인서 제출 필수(대표자 본인이 수강 시에도 제출)</b></p></c:if>
									</td>
								</tr>
								<tr>
									<th>낚시터명칭</th>
								...
								<tr>
									...
									<td>
										...
										<c:if test="${status.count eq '1'}">
											...
											data-crs-law-type="${info_crs.CRS_LAW_TYPE }">
