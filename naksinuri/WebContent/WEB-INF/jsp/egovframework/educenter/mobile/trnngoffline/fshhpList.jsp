...
<%-- <div class="pd0px">
	<%@ include file="../../../direct/mobile/left_menu.jsp" %>
</div> --%>
<style>
...
<form:form commandName="eduCenterMainVO" id="listForm" name="listForm" method="post">
	...
	<input type="hidden" id="CRS_MBR_CD" name="CRS_MBR_CD" value="CIDN010300"/>
	<div class="content respon3">
		<section id="writeForm" class="write_box">
			<div class="list_searchbox" style="text-align:right;">
				<select class="basic_select" title="검색조건선택" name="basic_select">
					<option value="select_total" <c:if test="${basic_select eq 'select_total' }">selected</c:if> >전체</option>
					<option value="select_crs_nm" <c:if test="${basic_select eq 'select_crs_nm' }">selected</c:if>>교육명</option>
					<option value="select_crs_addr" <c:if test="${basic_select eq 'select_crs_addr' }">selected</c:if>>교육장소</option>
				</select>
				<input type="text" class="basic_input" placeholder="검색어를 입력하세요" title="검색어입력" name="search_word" value="${search_word}" style="width:150px;"/>
				<button class="searchBtn" style="margin:0px; padding:0px 7px;"><span class="blind">검색</span><i class="fa fa-search" aria-hidden="true" ></i></button>
			</div>
			...
			<div class="">
				...
				<div class="total_num" style="font-size:14px;">전체 <b class="colorSky">${paginationInfo.totalRecordCount}</b>건의 낚시어선 오프라인 교육이 있습니다.(<b class="colorSky">${paginationInfo.currentPageNo}</b>/${total_page})</div>
			</div>
			
			<div class="board_list">
				<table class="list_tbl">
					<caption>오프라인 교육수강일정(낚시어선)</caption>	
					<colgroup>
						<col width="45%"/>
						<col width="40%"/>
						<col width="15%"/>
					...
					<thead>
						<tr>
							<th style="border-bottom:1px solid #e0e0e0;">교육명</th>
							<th style="border-bottom:1px solid #e0e0e0;">교육일시</th>
							<th style="border-bottom:1px solid #e0e0e0;">신청<br>인원</th>                                                                                                                                                                                                                                                                                                                                     </th>
						</tr>
						<tr>
							<th>교육장주소</th>
							<th>교육장소명</th>
					...
					<tbody>
						<c:if test="${empty list}">
							<tr><td colspan="5" class="text-center table-active">현재 신청가능한 오프라인 교육이 없습니다.</td></tr>
			           	...
			           	<c:forEach var="item" varStatus="status" items="${list}">
							...
							<tr>
								<td class="text-center" style="border-bottom:1px solid #666;">${item.CRS_ADDR}</td>
								<td class="text-center" style="border-bottom:1px solid #666;">${item.CRS_PLACE}</td>
								<td class="text-center" style="border-bottom:1px solid #666;">
									<c:choose>
...
function fnLinkPage(pageNo){
    ...
    document.listForm.action = "<c:url value='/educenter/m/trnngoffline/fshhpList.do'/>";
