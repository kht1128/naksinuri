...
function fnSelectInfs3(category){
	...
	if($('#category').val()==""){
		$("#frm").attr("action", "${pageContext.request.contextPath}/admin/info/angling/list.do");	
	}else{
		$("#frm").attr("action","${pageContext.request.contextPath}/admin/info/angling/riverlist.do");
	}
