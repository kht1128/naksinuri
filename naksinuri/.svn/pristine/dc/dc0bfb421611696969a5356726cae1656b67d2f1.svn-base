...
<input type="hidden" id="MBR_ID" name="MBR_ID" value="${parentInfo2.MBR_ID}"/>
              		...
              		<div class="row">				  	
              			<div class="col-lg-12 text-right">
	              			<a class="btn btn-danger btn-act-send-sms" href="javascript:void(0);">
				        		...
				        		<span class="hidden-sm-down">동영상수강링크 문자보내기</span>
				        	</a>
			        	</div>
              		</div>
		            	...
		            	<c:forEach var="item" varStatus="status" items="${list}">
		            		...
		            		<tr class="<c:if test="${status.index%2 eq 0}">table-active</c:if>">
		            			<td class="text-middle hide-cell-exceldown">
			            			<span class="checkbox-custom checkbox-primary">
	                              		<input class="selectable-item btn-check-item" type="checkbox" name="checkbox_item" value="${item.HMBR_DTL_SN}"><label></label>
	                            	</span>
                            	</td>
				                ...
				                <td class="text-middle text-center">		                 			
				                	<c:choose>
				                		...
				                		<c:otherwise>현재 수강중</c:otherwise>				                	
				                	</c:choose>
				                </td>
				                <td class="text-middle text-center">
	...
	<input type="hidden" name="chkedMbrIds" value="${parentInfo2.MBR_ID}"/>
	<input type="hidden" name="chkedHMbrDtlSns" value=""/>
	</form:form>
    ...
    function pageMovePrev() {
    	...
    	form.HMBR_SN.value = '';
    	form.CRS_DTL_SN.value = '';
    	form.HMBR_DTL_SN.value = '';
    	form.TRN_SN.value = '';
    	form.MBR_ID.value = '';
    ...
    $(".btn-act-send-sms").click(function() {
		...
		$("input[name=checkbox_item]:checked").each(function() {
			if(chkedHMbrDtlSns.length!=0) chkedHMbrDtlSns+=",";
			chkedHMbrDtlSns+=$(this).val();
		});
		if(chkedHMbrDtlSns.length==0) {
			alertify.alert("교과목(동영상)이 선택되지 않았습니다.<br>동영상수강링크 문자를 보내실 항목을 선택해주세요.");
			return;
		}
    	var form = document.getElementById('ajaxSendSmsForm');
    	form.chkedHMbrDtlSns.value = chkedHMbrDtlSns;
    	$.ajax({
			type:"POST",
