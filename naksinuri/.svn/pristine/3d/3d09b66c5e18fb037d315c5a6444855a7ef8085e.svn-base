				...
				<div class="businessSlide">
					<ul>
						<c:forEach var="simges" items="${simglist}" varStatus="status">
							<li style="text-align:center;background-color:#f3f3f3">
								<img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${simges.san_simg}"/>&fileSn=<c:out value="${simges.file_sn}"/>' alt='' style="width:auto;height:540px;margin:0 auto;" />
							</li>
						</c:forEach>
					</ul>
					<script>
