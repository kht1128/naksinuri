...
<input type="hidden" name="pageUnit" id="pageUnit" value="${pageUnit }" />
	<div id="noFishing" class="content respon2">
		<div class="tabArea"> <!-- 탭이 3개면 class="tabArea tab3" 4개면 뭐겠냐 -->
			<ul class="floats">
				<li id="plocation" ><a href="/promotion/plocation/list.do">낚시금지구역</a></li>
				<li id="llocation" class="on"><a href="/promotion/llocation/list.do">낚시제한구역</a></li>
			</ul>
		</div>
...
-->
		...
		<section id="nofishingList" class="floats">
			<div class="listbox" id="llolist">
				<c:if test="${fn:length(select_list) eq 0}">
					<div class="list_con">데이터가 없습니다.</div>
				</c:if>
				...
				<div class="list_con">
					...
					<dl>
						<dt><a href="#;" onclick="mapview(${item.x_latitude},${item.x_hardness})"onclick="mapview(${item.x_latitude},${item.x_hardness})"><em>${item.x_name}</em><span><c:choose>
						...
						<dd>
							<ul>
								<li><em>지정기간</em><span>${item.x_term} ~</span></li>
...
$('document').ready(function() {
 ...
 $("select[name^=sido]").change(function() {
	 ...
	 $.ajax({
		 type:"post",
		 url:"${contextPath}/promotion/llocation.do",
		 datatype:"json",
			 ...
			 if(data.length > 0){
				 for(var i=0;i<=data.length;i++){
					...
					*/
					...
					$('#llolist').append('<div class="list_con"><div class="num">'+(i+1)+'</div>\
							<dl>\
								<dt><a href="#;" id='+i+' onclick="mapview('+data[i].x_latitude+','+data[i].x_hardness+')"onclick="mapview('+data[i].x_latitude+','+data[i].x_hardness+')"><em>'+data[i].x_name+'</em><span>'+(data[i].x_rang.length>16?data[i].x_rang.substr(0,15)+"...":data[i].x_rang)+'</span></a></dt>\
									<dd>\
										<ul>\
											<li><em>지정기간</em><span>'+data[i].x_term+' ~</span></li>\
			 ...
			 if(data.length > 0){
				 for(var i=0;i<=data.length;i++){
					...
					*/
					...
					$('#llolist').append('<div class="list_con active"><div class="num">'+(i+1)+'</div>\
							<dl>\
