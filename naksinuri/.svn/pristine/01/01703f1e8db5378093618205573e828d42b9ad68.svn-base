...
<table id="datalist" class="table table-hover footable footable-paging footable-paging-center">
	...
	<tr class="<c:if test="${status.index%2 eq 0}">table-active</c:if>">
		...
		<td class="text-middle text-center">
			<a href="#;" class="btn btn-outline btn-default btn-sm "onclick="clk_view_data('${item.MBR_ID}')">자세히보기</a>
			<%-- <button type="button" class="btn btn-outline btn-success btn-xs clk_mod_mbr_data_modal"  
...
<div class="form-group row">
	<div class="col-md-12">
		...
		<label class="form-control-label text-danger text-left font-size-12">회원을 선택하시면 이곳에 자동으로 추가됩니다.<br>선택 후 삭제한 회원은 재검색 하시면 다시 불러올 수 있습니다.</label>
	</div>
</div>
...
/*
$(".clk_mod_mbr_data_modal").click(function() {
	...
	$.ajax({
		...
		success: function(data, status, xhr) {
			//console.log('success!');
			$("#admPublicMultiModal").html(data);
			$("#admPublicMultiModal").modal({backdrop: 'static', keyboard: false},'show');
		},
		complete : function() {
			//console.log('complete!');
	    },
