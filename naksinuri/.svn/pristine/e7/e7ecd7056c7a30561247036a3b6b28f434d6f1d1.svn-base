...
<form:form commandName="eduCategoryInsInfVO" id="listForm" name="listForm" method="post" enctype="multipart/form-data" onsubmit="return frm_submit()" 
	action="/eduadm/category/academy/modify_act.do"> 
			...
			<div class="panel-body">
		           	...
		           	<div class="col-md-10">
		            	<select class="form-control selectpicker_manual" data-style="btn-outline btn-default" id="search_sel_area_sido" name="SIDO_CD" required>
							...
							<c:forEach var="item_category" items="${list_address_cd}">
								<option value="${item_category.CD_ID}" <c:if test="${item_category.CD_ID eq info.SIDO_CD }">selected</c:if> >${item_category.CD_NM}</option>
							</c:forEach>	        			
	      				</select>
					</div>
				</div>	
		           	...
		           	<div class="col-md-10">
		            	<select class="form-control selectpicker_manual" data-style="btn-outline btn-default" id="search_sel_area_signgu" name="SIGNGU_CD" required>
							...
							<c:forEach var="item_category" items="${list_address_signgu_cd}">
								<option value="${item_category.CD_ID}" <c:if test="${item_category.CD_ID eq info.SIGNGU_CD }">selected</c:if> >${item_category.CD_NM}</option>
							</c:forEach>	        			
	      				</select>
					</div>
				</div>	
				<div class="form-group row">
					<label class="col-md-2 form-control-label" >교육기관명 <span class="red-600">*</span></label>
		           	<div class="col-md-10">
		            	<input type="text" class="form-control bg-white" name="CAT_INS_NM" placeholder="교육기관명을 입력하세요."  value="<c:out value="${info.CAT_INS_NM}"/>" required>
					</div>
				</div>
				<div class="form-group row">
					<label class="col-md-2 form-control-label" >대표자직책 <span class="red-600">*</span></label>
		           	<div class="col-md-10">
		            	<input type="text" class="form-control bg-white" name="CAT_INS_PSTN" placeholder="대표자직책을 입력하세요." value="<c:out value="${info.CAT_INS_PSTN}"/>" required>
					</div>
				</div>
				<div class="form-group row">
					<label class="col-md-2 form-control-label" >대표자</label>
		           	<div class="col-md-10">
		            	<input type="text" class="form-control bg-white" name="CAT_INS_CEO" placeholder="대표자를 입력하세요." value="<c:out value="${info.CAT_INS_CEO}"/>">
					</div>
				</div>
				<div class="form-group row">
					<label class="col-md-2 form-control-label" >대표번호</label>
		           	<div class="col-md-10">
		            	<input type="text" class="form-control bg-white" name="CAT_INS_TEL" placeholder="대표번호를 입력하세요." value="<c:out value="${info.CAT_INS_TEL}"/>" numberOnly>
					</div>
				</div>
				<div class="form-group row">
					<label class="col-md-2 form-control-label" >주소</label>
		           	<div class="col-md-10">
		            	<input type="text" class="form-control bg-white" name="CAT_INS_ADDR" placeholder="주소정보를 입력하세요." value="<c:out value="${info.CAT_INS_ADDR}"/>">
					</div>
				</div>
				<div class="form-group row">
					<label class="col-md-2 form-control-label">설명</label>
					<div class="col-md-10">
						<textarea class="form-control h-100" name="CAT_INS_DSCRP" placeholder="설명(요약)" row="5" ><c:out value="${info.CAT_INS_DSCRP}"/></textarea>
					</div>
				</div>
		   		<div class="form-group row">
					...
					<div class="col-md-6">
						<textarea class="form-control h-p100" id="CAT_INS_LOGO" name="CAT_INS_LOGO" placeholder="로고이미지의 바이너리 코드값을 입력하세요." row="5" >${info.CAT_INS_LOGO}</textarea>
					</div>
					<div class="col-md-2">
				...
				<div class="form-group row">
					...
					<div class="col-md-6">
						<textarea class="form-control h-p100" id="CAT_INS_STAMP" name="CAT_INS_STAMP" placeholder="직인이미지의 바이너리 코드값을 입력하세요." row="5" >${info.CAT_INS_STAMP}</textarea>
					</div>
					<div class="col-md-2">
						<a href="javascript:void(0)" class="btn btn-outline btn-warning clk_confirm_img_code mr-10" 
				...
				<div class="form-group row">
					...
					<div class="col-md-6">
						<textarea class="form-control h-p100" id="CAT_INS_OTLNMAP" name="CAT_INS_OTLNMAP" placeholder="교육장약도이미지의 바이너리 코드값을 입력하세요." row="5" >${info.CAT_INS_OTLNMAP}</textarea>
					</div>
					<div class="col-md-2">
						<a href="javascript:void(0)" class="btn btn-outline btn-warning clk_confirm_img_code mr-10" 
				...
				<div class="form-group row">
					...
					<div class="col-md-10">
						<div class="radio-custom radio-default radio-inline">
							<input type="radio" id="USE_ST_Y" name="USE_ST" value="1" <c:if test="${info.USE_ST eq '1'}">checked</c:if>>
							<label for="USE_ST_Y">사용함</label>
						</div>
						<div class="radio-custom radio-default radio-inline">
							<input type="radio" id="USE_ST_N" name="USE_ST" value="0" <c:if test="${info.USE_ST eq '0'}">checked</c:if>>
				...
				<div class="form-group row">
		        	<label class="col-md-2 form-control-label">작성일자</label>
		           	<div class="col-md-10">
				...
				<div class="form-group row">
					<label class="col-md-2 form-control-label">변경일자</label>
					<div class="col-md-10">
