...
<%@include file="../admin_leftTab.jsp"%>
	<div id="container">
		...
		<div id="content">			
			...
			<h1 class="bigTit"><i class="fa fa-trash" aria-hidden="true"></i> 고객의소리 휴지통 <div class="trash_box"><%@include file="../trash_selectBox.jsp" %></div></h1>
				...
				<div class="t_list_area">
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${voc_list}">
							<tr>
								<td><input type="checkbox" name="delYn" class="check2" value="${item.voc_no}"></td>
								...
								<td class="textcenter"><a href="#;" onclick="view2('${item.voc_no}')">${item.voc_phone}</a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.voc_email}')">${item.voc_email}</a></td>						
								<td class="textcenter"><a href="#;" onclick="view2('${item.voc_no}')">${item.voc_insert_dt}</a></td>		
							</tr>
						</c:forEach>
						</tbody>
					</table>
