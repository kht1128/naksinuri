...
<form:form commandName="errorVO" id="listForm" name="listForm" method="post">
      ...
      <div class="page-header">
      	...
      	<div class="page-header-actions">
        	<a class="btn btn-outline btn-info excel-down btn-act-external-excel-down" href="javascript:void(0);" 
        		data-excel-type="/adm/log/listSysEduadm.do"
	  				...
	  				<div class="input-group col-lg-12 p-0">
						...
						<div class="col-lg-4 p-0 pl-10 text-right">
							<div class="btn-group w-p100">
								<input type="text" class="form-control input-auto-enter-key" name="searchKeyword" placeholder="검색어를 입력하세요." autocomplete="off" value="${searchKeyword}" data-toggle="tooltip" data-original-title="요청,응답에러 등을 입력해주세요.">
					...
					<!-- table:start -->
	          		...
	          		<thead>
	          			<tr>
							...
							<th class="text-middle text-center ">발생일자</th>
							<th class="text-middle text-center ">아이디<br>/요청아이피</th>
							<th class="text-middle text-center ">요청</th>
							<th class="text-middle text-center ">응답(에러)</th>	
	                	</tr>       				
	             	</thead>
					<tbody>
		            	...
		            	<c:forEach var="item" varStatus="status" items="${list}">
		            		...
		            		<tr class="<c:if test="${status.index%2 eq 0}">table-active</c:if>">
		            			...
		            			<td class="text-middle text-center">${item.ERR_DT }</td>
		            			<td class="text-middle text-center">
		            				<c:choose>
		            					<c:when test="${not empty item.MBR_ID }">${item.MBR_NCNM }<br>(${item.MBR_NM })</c:when>
		            					<c:otherwise>비로그인</c:otherwise>
		            				</c:choose> 
		            				<br><span class="grey-400">${item.ERR_IP }</span></td>
		            			<td class="text-middle text-left">
		            				<c:choose>
		            					<c:when test="${item.ERR_SERVLET_NM ne 'action' }"><c:out value="${fn:trim(item.ERR_EXP_MSG)}" escapeXml="false"/></c:when>
		            					<c:otherwise><c:out value="${fn:trim(item.ERR_REQ_URI)}" escapeXml="false"/></c:otherwise>
		            				</c:choose>
		            			</td>
		            			<td class="text-middle text-left">
	            					<c:choose>
		            					<c:when test="${item.ERR_EXP eq 'none' }">(에러코드 : ${item.ERR_STAT_CD}) 오류가 발생되었습니다.</c:when>
		            					<c:otherwise>(에러코드 : ${item.ERR_STAT_CD}) <c:out value="${fn:trim(item.ERR_EXP.replaceAll('<','').replaceAll('>',''))}" escapeXml="false"/></c:otherwise>
		            				</c:choose>
		            			</td>	
		              		</tr> 
		            	</c:forEach>
...
function pageMove(obj) {
	...
	form.action = "/adm/log/listSys{addWebLink}.do";
	form.submit();
}
...
$(".clk-btn-detail-show").click(function() {
	var targetId = $(this).attr('data-btn-id');
	$(targetId).show();
}); 
</script>
