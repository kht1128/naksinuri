...
function fnSelectInfs(pageIndex) {
	...
	$("#frm").attr("action", "/info/industry/list.do");
	$("#frm").submit();
			...
			<div class="search_box fish_business">
				...
				<div class="scon">
					<dl>
						<dt>· 구분</dt>
						<dd>
							<input type="checkbox" id="san_aag" name="san_aag" value="협회/기관/단체" title="협회/기관/단체" <c:if test ="${san_aag eq'협회/기관/단체'}">checked</c:if>/><label for="san_aag"><span tabindex="0" onkeyPress="if (event.keyCode==13 || event.keyCode==32){$('#san_aag').click();}"></span>협회/기관/단체</label>
							<input type="checkbox" id="san_zogu" name="san_zogu" value="조구업체" title="조구업체" <c:if test ="${san_zogu eq'조구업체'}">checked</c:if>/><label for="san_zogu"><span tabindex="0" onkeyPress="if (event.keyCode==13 || event.keyCode==32){$('#san_zogu').click();}"></span>조구업체</label>
							<input type="checkbox" id="san_media" name="san_media" value="미디어"  title="미디어" <c:if test ="${san_media eq '미디어'}">checked</c:if>/><label for="san_media"><span tabindex="0" onkeyPress="if (event.keyCode==13 || event.keyCode==32){$('#san_media').click();}"></span>미디어</label>
							<input type="checkbox" id="san_sell" name="san_sell" value="판매점" title="판매점"  <c:if test ="${san_sell eq'판매점'}">checked</c:if>/><label for="san_sell"><span tabindex="0" onkeyPress="if (event.keyCode==13 || event.keyCode==32){$('#san_sell').click();}"></span>판매점</label>
							<input type="checkbox" id="san_chool" name="san_chool" value="출조점"  title="출조점" <c:if test ="${san_chool eq '출조점'}">checked</c:if>/><label for="san_chool"><span tabindex="0" onkeyPress="if (event.keyCode==13 || event.keyCode==32){$('#san_chool').click();}"></span>출조점</label>
							<input type="checkbox" id="san_inprov" name="san_inprov" value="낚시정보제공" title="낚시정보제공" <c:if test ="${san_inprov eq'낚시정보제공'}">checked</c:if>/><label for="san_inprov" ><span tabindex="0" onkeyPress="if (event.keyCode==13 || event.keyCode==32){$('#san_inprov').click();}"></span>낚시정보제공</label>
						</dd>
					</dl>
					<dl>
						<dt>· 시/도</dt>
						<dd>
							<select class="search_select" name="searchSido" id="searchSido" title="시,도 선택옵션">
					...
					<dl>
						<dt>· 시/군/구</dt>
						<dd>
							<select class="search_select"  name="searchGugun" id="searchGugun" title="시,군,구 선택옵션">
					...
					<dl>
						<dt>· 상호명</dt>
						<dd><input type="text"  name="searchText1" class="search_input" value="${searchText1}" title="상호명"/></dd>
					</dl>
					<dl>
						<dt>· 홈페이지</dt>
						<dd><input type="checkbox" id="website" name="website" title="홈페이지"/><label for="website"><span></span>자체홈페이지 여부</label></dd>
					</dl>
<!--  
					<dl>
						<dt>· 폐업</dt>
...
-->		
		...
		<section id="galleryList" class="list_box">
 			...
 			<div class="tabArea tab7 mbNone">	<!-- 탭이 3개면 class="tabArea tab3" 4개면 뭐겠냐 --> 
				<ul class="floats" id="cata">
					<li id="allca" class="on"><a href="/info/industry/list.do" ${category_type == "all" ? 'title="전체 선택됨"' : 'title="전체"'}>전체</a></li>
					<li id="aagca"><a href="#;" onclick="aagfind()" ${category_type == "san_aag" ? 'title="협회/기관/단체 선택됨"' : 'title="협회/기관/단체"'}>협회/기관/단체</a></li>
					<li id="zoca"><a href="#;" onclick="zogufind()" ${category_type == "san_zogu" ? 'title="조구업체 선택됨"' : 'title="조구업체"'}>조구업체</a></li>
					<li id="meca"><a href="#;" onclick="mediafind()" ${category_type == "san_media" ? 'title="미디어 선택됨"' : 'title="미디어"'}>미디어</a></li>
					<li id="selca"><a href="#;" onclick="sellfind()" ${category_type == "san_sell" ? 'title="판매점 선택됨"' : 'title="판매점"'}>판매점</a></li>
					<li id="choolca"><a href="#;" onclick="choolfind()" ${category_type == "san_chool" ? 'title="출조점 선택됨"' : 'title="출조점"'}>출조점</a></li>
					<li id="inpovca"><a href="#;" onclick="inpfind()" ${category_type == "san_inprov" ? 'title="낚시정보제공 선택됨"' : 'title="낚시정보제공"'}>낚시정보제공</a></li>
				</ul>
			</div>
					
			<div class="sortbox">
				<ul class="floats">
					<li id="ids" class="on"><a href="#;" onclick="order_id()" ${searchType == "id" || empty(searchType) ? 'title="등록순 선택됨"' : 'title="등록순"'}>등록순</a></li>					
					<li id="kor"><a href="#;" onclick="order_kor()" ${searchType == "kor" ? 'title="한글순 선택됨"' : 'title="한글순"'}>한글순</a></li>
					<li id="views"><a href="#;" onclick="order_views()" ${searchType == "views" ? 'title="조회수순 선택됨"' : 'title="조회수순"'}>조회순</a></li>
				</ul>
				<p class="totalNum" style="right:145px;">전체 <b class="colorSky">${ind_total}</b>건의 게시물이 있습니다. (<b class="colorSky">${pageIndex}</b>/${pagesize})</p>
				<div id="btnArea" class="noupline" style="right: 0px; position: absolute; top: 3px; margin-top:0px;">
					<ul class="floats">
						<li class="fr"> 
							<a href="/info/industry/login.do" class="btn_report btn_red">정보등록신청</a>
						</li>
					</ul>
				</div>
			</div>
					...
					<li>
						<c:choose>
							<c:when test="${item.orignl_file_nm ne null }">
								<a href="#;" onclick="view(${item.san_sn})" class="pic" style="line-height:206px"><img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${item.san_img}"/>&fileSn=<c:out value="${item.file_sn }"/>' alt='<c:out value="${item.san_name }"/>' /></a>
							</c:when>
							<c:otherwise>
								<a href="#;" onclick="view(${item.san_sn})" class="pic"><img src="/naksinuri_original/common_main/img/noImage_big.png" alt='<c:out value="낚시누리"/>' height="100%"/></a>
							</c:otherwise>
						</c:choose>
						<a href="#;" onclick="view(${item.san_sn})"class="subject">
							<span class="cate">
							<c:set var="san_car" value="${item.san_zogu} ${item.san_media} ${item.san_sell} ${item.san_chool} ${item.san_inprov} ${item.san_aag}"/>
							[${fn:substring(san_car,0,19)}
							<%-- <c:if test="${fn:length(san_car)>20}">...</c:if> --%>]
							</span>
							<em>${fn:substring(item.san_name,0,12)}
								<%-- <c:if test="${fn:length(item.san_name)>11}">...</c:if> --%>
							</em>
							<span class="bestfish"><b>사업영역 : </b>${fn:substring(item.san_item,0,13)}
								<%-- <c:if test="${fn:length(item.san_item) > 12 }">...</c:if> --%>
							</span>
						</a>
						<a href="#;" onclick="view(${item.san_sn})" class="search_square"><i class="fa fa-search" aria-hidden="true" title="${item.san_name} 상세보기"></i></a>
...
$('#san_sn').val(san_sn);
$("#veiw_go").attr("action", "/info/industry/ind_find.do");
$('#veiw_go').submit();
...
function aagfind() {
	...
	$('input:checkbox[id="san_aag"]').attr("checked", true);
	$('input:checkbox[id="san_zogu"]').attr("checked", false);
	$('input:checkbox[id="san_media"]').attr("checked", false);
	$('input:checkbox[id="san_sell"]').attr("checked", false);
	$('input:checkbox[id="san_chool"]').attr("checked", false);
	$('input:checkbox[id="san_inprov"]').attr("checked", false);
	$("#frm").attr("action", "/info/industry/list.do");
	$("#frm").submit();
	//페이징 숫자 버튼 눌렀을때 이 함수가 호출되면서 서브밋으로 주소에 포스트로 값 넘긴다..
}
...
function zogufind() {
	...
	$('input:checkbox[id="san_zogu"]').attr("checked", true);
	$('input:checkbox[id="san_aag"]').attr("checked", false);
	$('input:checkbox[id="san_media"]').attr("checked", false);
	$('input:checkbox[id="san_sell"]').attr("checked", false);
	$('input:checkbox[id="san_chool"]').attr("checked", false);
	$('input:checkbox[id="san_inprov"]').attr("checked", false);
...
function mediafind() {
	...
	$('input:checkbox[id="san_zogu"]').attr("checked", false);
	$('input:checkbox[id="san_aag"]').attr("checked", false);
	$('input:checkbox[id="san_media"]').attr("checked", true);
	$('input:checkbox[id="san_sell"]').attr("checked", false);
	$('input:checkbox[id="san_chool"]').attr("checked", false);
	$('input:checkbox[id="san_inprov"]').attr("checked", false);
	$("#frm").attr("action", "/info/industry/list.do");
...
function sellfind() {
	...
	$('input:checkbox[id="san_aag"]').attr("checked", false);
	$('input:checkbox[id="san_media"]').attr("checked", false);
	$('input:checkbox[id="san_sell"]').attr("checked", true);
	$('input:checkbox[id="san_chool"]').attr("checked", false);
	$('input:checkbox[id="san_inprov"]').attr("checked", false);
	$("#frm").attr("action", "/info/industry/list.do");
	$("#frm").submit();
...
function choolfind() {
	...
	$('input:checkbox[id="san_media"]').attr("checked", false);
	$('input:checkbox[id="san_sell"]').attr("checked", false);
	$('input:checkbox[id="san_chool"]').attr("checked", true);
	$('input:checkbox[id="san_inprov"]').attr("checked", false);
	$("#frm").attr("action", "/info/industry/list.do");
	$("#frm").submit();
	//페이징 숫자 버튼 눌렀을때 이 함수가 호출되면서 서브밋으로 주소에 포스트로 값 넘긴다..
...
function inpfind() {
	...
	$('input:checkbox[id="san_sell"]').attr("checked", false);
	$('input:checkbox[id="san_chool"]').attr("checked", false);
	$('input:checkbox[id="san_inprov"]').attr("checked", true);
	$("#frm").attr("action", "/info/industry/list.do");
	$("#frm").submit();
	//페이징 숫자 버튼 눌렀을때 이 함수가 호출되면서 서브밋으로 주소에 포스트로 값 넘긴다..
}
