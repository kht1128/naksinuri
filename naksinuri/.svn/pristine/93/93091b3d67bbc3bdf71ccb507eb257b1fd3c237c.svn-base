...
<form:form commandName="eduMemberVO" id="listForm2" name="listForm2" class="modal-content form-horizontal" method="post">
	<input type="hidden" name="MBR_LV_ID" value="${MBR_LV_ID}" />
	<input type="hidden" name="MBR_GRP_ID" value="${MBR_GRP_ID}" />
	<input type="hidden" name="MBR_POSITION_CD" value="${MBR_POSITION_CD}" />
...
function fn_egov_link_page_2(pageNo){
	...
	form.action = '';
   	$.ajax({
		type:"POST",
		url :"/seadm/member/util/ajaxlist.do",
...
$("#btn_submit").click(function() {
	$.ajax({
		type:"POST",
		url :"/seadm/member/auth/write_act.do",
		data:$('#listForm2').serialize(),
		dataType: "json",
		contentType: "application/x-www-form-urlencoded;charset=UTF-8",//"application/json;charset=UTF-8",
		async: true,
