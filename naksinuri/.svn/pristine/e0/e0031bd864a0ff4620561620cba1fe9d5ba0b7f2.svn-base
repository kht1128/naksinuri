...
<head>
	...
	<link rel="stylesheet" type="text/css" href="/naksinuri_original/common_main/css/common.css?ver=20221213" />
	<link rel="stylesheet" type="text/css" href="/naksinuri_original/common_main/css/base.css?ver=20191217"/>
...
<c:choose>
	...
	<c:when test="${urlcontroller eq 'educenter' and urlmethodname eq 'trnngoffline' and urlmethodsubname eq '' and urlpagename eq 'fshlcList'}">
		<c:set var="paName" value="오프라인 교육(낚시터) 일정"/>
		<c:set var="pageMode" value=""/>
		<c:set var="groupName" value="낚시전문교육"/>
		<c:set var="depthName" value="오프라인 교육(낚시터) 신청"/>		
		<c:set var="pageName" value="오프라인 교육(낚시터) 일정"/>
		<c:set var="menuNum" value="10"/>
	</c:when>
	<c:when test="${urlcontroller eq 'educenter' and urlmethodname eq 'trnngoffline' and urlmethodsubname eq '' and urlpagename eq 'fshhpList'}">
		<c:set var="paName" value="오프라인 교육(낚시어선) 일정"/>
		<c:set var="pageMode" value=""/>
		<c:set var="groupName" value="낚시전문교육"/>
		<c:set var="depthName" value="오프라인 교육(낚시어선) 신청"/>		
		<c:set var="pageName" value="오프라인 교육(낚시어선) 일정"/>
		<c:set var="menuNum" value="10"/>
	</c:when>
	...
	<c:when test="${pageName eq 'fishjobWrite' }">
		<c:set var="paName" value="낚시터정보 > 낚시터 정보등록"/>
	</c:when>
...
<script type="text/javascript">
    ...
    var position = sessionStorage.getItem('naksi_device');
    //< 모바일인 경우
    if( isMobile && position !='pc' )
    {
    	var url = window.location.href;
    	if(url.search("/educenter/") !== -1) {
    		url = url.replace('/educenter/','/educenter/m/');
    		location.href = url;
    	} else {
