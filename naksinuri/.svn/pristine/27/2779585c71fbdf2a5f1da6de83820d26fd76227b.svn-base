...
<div id="wrapper">
	...
	<div id="container">
		...
		<div id="content">			
			...
			<h1 class="bigTit"><i class="fa fa-trash" aria-hidden="true"></i> 종료된 설문조사 휴지통 <div class="trash_box"><%@include file="../trash_selectBox.jsp" %></div></h1>

			<section id="table-list">
				<!-- 탭 영역 { -->
				<div id="tabarea"> 
					<ul class="floats">
						<li><a href="${pageContext.request.contextPath}/admin/survey/survey/trash.do" class="survey_now">진행중인 설문조사</a></li>
						<li><a href="${pageContext.request.contextPath}/admin/survey/survey/endtrash.do"  class="survey_end on">종료된 설문조사</a></li>
					</ul>
				</div>
				<!-- } 탭 영역 -->
				...
				<div class="t_list_area">
					...
					<table class="t_list">
						...
						<c:forEach var="item" items="${select_list}">
							<tr>
								<td><input type="checkbox" name="delYn" class="check2" value="${item.sv_id}"><input type="hidden" name="sv_id" id="bo_sn" value="${item.sv_id }"/></td>
								...
								<td class="textcenter"><a href="#;" onclick="view2('${item.sv_id}')">${item.reg_date }</a></td>		
