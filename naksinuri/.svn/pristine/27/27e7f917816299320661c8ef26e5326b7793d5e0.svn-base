...
<c:choose>
	<c:when test="${empty info}">
		<c:if test="${not empty IS_TABLE_TR and IS_TABLE_TR eq 'Y'}"><%-- 모달이 아닌 경우 --%>
			<tr><td colspan="2">해당 교육이 존재하지 않거나<br>더 이상 사용하지 않는 교육정보 입니다.</td></tr>
		</c:if>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content form-horizontal">
					<div class="modal-header">
				 			...
				 			<h4 class="modal-title">교육정보</h4>
					</div>
					<div class="modal-body pt-0 pl-30 pr-30">
						<div class="form-group" >
							<p class="pt-30 pb-30 pl-5 pr-5 text-center">해당 교육이 존재하지 않거나<br>더 이상 사용하지 않는 교육정보 입니다.</p>
						</div>
	...
	<c:otherwise>
		...
		<c:if test="${empty IS_TABLE_TR}">
			...
			<form id="modal_action_form" class="modal-content form-horizontal" method="post" autocomplete="off" 
				...
				<div class="modal-body pt-0 pl-20 pr-20">
					...
					<div class="text-right">
						...
						<c:if test="${isOpened eq true and info.LOCK_ST eq '0'}">
							<button type="button" class="btn btn-primary btn-outline clk_btn_act" onclick="checkEdu('${info.CRS_SN}')">신청하기</button>
						</c:if>
			            <button type="button" class="btn btn-default btn-outline" data-dismiss="modal">취소(닫기)</button>
			        </div>
			  	</div>
