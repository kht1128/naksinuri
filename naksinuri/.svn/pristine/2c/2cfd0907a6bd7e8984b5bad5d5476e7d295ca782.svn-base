	...
	<div id="customerSound" class="content respon3">
		<section id="writeForm" class="write_box">
			...
			<dl>
				...
				<span class="txt_red">개인정보보호를 위해 실명기재를 금지합니다.</span></dd>
			</dl>
			<dl>
				<dt>* 비밀번호</dt>
				<dd><input type="password" placeholder="비밀번호를 입력해주세요." id="bo_pass" name="bo_pass" ></dd>
			</dl>
...
if ($("#bo_content").val() == " <p>&nbsp;</p>") {
	...
	try {
		...
		if ($('#bo_sn').val() != '') {
				$("#imform")
						.attr("action", "/share/usage/m/update_data.do");
				$("#imform").submit();
			} else {
				$("#imform")
						.attr("action", "/share/usage/m/insert_data.do");
				document.getElementById("imform").submit();
			}
