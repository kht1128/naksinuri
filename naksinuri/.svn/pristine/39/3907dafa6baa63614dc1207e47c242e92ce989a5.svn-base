		...
		<section id="writeForm" class="write_box">
			...
			<div class="owner_box">
				...
				<dl>
					...
					<dd>
						<input type="password" id="mbr_pwd" name="mbr_pwd" class="write_input w100" placeholder="암호를 입력해주세요." autocomplete="off" value="<c:if test="${info_gongmo ne null and not empty info_gongmo.mbr_pwd}">${info_gongmo.mbr_pwd}</c:if>" />
						<span class="txt_red">
			...
			<div class="owner_box">
				<dl>
					...
					<dd>
						<input type="text" class="write_input w100" placeholder="성명을 입력해주세요." id="mbr_name" name="mbr_name" value="<c:if test="${info_gongmo ne null and not empty info_gongmo.mbr_name}"><c:out value="${info_gongmo.mbr_name}"/></c:if>"/>
					</dd>
				</dl>
				<dl>
				...
				<dl>
					...
					<dd>
						...
						<select class="naksi_select w30" id="frm_phone1" name="frm_phone1" >
						...
						<input type="text" class="write_input w30" name="frm_phone2" maxlength="4"  id="frm_phone2" value="<c:if test="${info_gongmo ne null and not empty info_gongmo.mbr_hp}">${mbr_hp_arr[1]}</c:if>"/> - 
						<input type="text" class="write_input w30" name="frm_phone3" maxlength="4"  id="frm_phone3" value="<c:if test="${info_gongmo ne null and not empty info_gongmo.mbr_hp}">${mbr_hp_arr[2]}</c:if>"/>
					</dd>                                                                                                                           
				</dl>
				<dl>
					<dt>이메일</dt>
					<dd>
						...
						<input type="text" class="write_input w30" placeholder="이메일을 입력하세요" name="frm_email1" id="frm_email1" value="<c:if test="${info_gongmo ne null and not empty info_gongmo.mbr_email}">${mbr_email_arr[0]}</c:if>"> @
						<select id="frm_email_addr" class="write_input w60" onchange="chgaddr_email();">					
						...
						<input type="text" style="<c:if test="${info_gongmo eq null and empty info_gongmo.mbr_email and mbr_email_self eq null}">display:none</c:if>" class="write_input w100" placeholder="이메일을 입력하세요" id="frm_email2" value="<c:if test="${info_gongmo ne null and not empty info_gongmo.mbr_email}">${mbr_email_arr[1]}</c:if>">
					</dd>
				</dl>
			</div>			
			...
			<div class="owner_box">			
				<dl>
					...
					<dd>
						<input type="text" class="write_input w100" placeholder="작품제목을 입력해주세요." id="work_subject" name="work_subject" value="<c:if test="${info_gongmo ne null and not empty info_gongmo.work_subject}"><c:out value="${info_gongmo.work_subject}"/></c:if>"/>
					</dd>
				</dl>
				<dl>
...
function cancel(){
	...
	$("#imform").attr("action", "/gongmo/gongmo/m/check.do");
	$("#bo_sn").val("");
...
function cancel_modify(){
	...
	$("#imform").attr("action", "/gongmo/gongmo/m/search.do");
	$("#bo_sn").val("");
	document.getElementById("imform").submit();
}
	...
	$.ajax({
	    ...
	    success: function (data) {
	    	if(data.error == '1') {
	    		...
	    		$("#imform").attr("action", "/gongmo/gongmo/m/mbrinsert_data.do");
	    		$("#imform").submit();
	    	}
	    },
	    error: function (e) {
	...
	$.ajax({
	    ...
	    success: function (data) {
	    	if(data.error == '1') {
	    		...
	    		$("#imform").attr("action", "/gongmo/gongmo/m/mbrupdate_data.do");
		    	$("#imform").submit();
	    	}
	    },
	    error: function (e) {
