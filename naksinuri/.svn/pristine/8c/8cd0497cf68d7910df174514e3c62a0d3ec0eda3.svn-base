...
jQuery(function ($) {
    ...
    var gnu = {
        url : '/se2/photo_uploader/popup/insert.do',
        container_el : 'body',
        dreg_area : '#drag_area',
        ...
        _delete : function(e){
            ...
            var othis = gnu,
                ...
                data_sn = $button.attr("data-sn");
            if( delete_url ){
                $.ajax({
                    //url: othis.url+"?del=1&file="+ delete_url
                	url: "/se2/photo_uploader/popup/delete.do"+"?del=1&file="+ delete_url + "&fid="+data_id+"&fsn="+data_sn
        ...
        _add : function(e, data, preload){
            ...
            $.each(data.files, function (index, file) {
                ...
                var node = $('<div/>')
                        ...
                        .append($('<span/>').addClass("delete_img").attr({"data-id":file._atchFileId,"data-sn":file._atchFileSn,"data-delete":file.name,"data-url":file.url}).html("<img src='./img/system_delete.png' alt='삭제' title='삭제' >")),
                ...
                if (file.url && !file.error) {
                    var ret = othis.get_ratio( file.width, file.height ),
                        ...
                        node
                            ...
                            .end().find(".delete_img").attr({"data-id":file._atchFileId,"data-sn":file._atchFileSn,"data-delete":file.name,"data-url":file.url});
                } else if (file.error) {
    ...
    $("#img_upload_submit").bind("click", function(e){
        ...
        $(gnu.dreg_area_list).find(".delete_img").each( function(i, f){
            ...
            aResult[j]['fid'] = $(this).attr("data-id");
            aResult[j]['fsn'] = $(this).attr("data-sn");            
            j++;
        });
        if( aResult.length ){
