...
function fnSelectInfs(pageIndex) {
	...
	$("#frm").attr("action", "${pageContext.request.contextPath}/info/admin/riverfishing.do");
	$("#frm").submit();
...
<input type="hidden" name="cate_mod_st" id="cate_mod_st" value="${info.cate_mod_st }" />
...
<%@include file="admin_leftTab.jsp"%>
	...
	<form:form commandName="riverform" id="riverform" name="listform" method="post" enctype="multipart/form-data" >
		...
		<input type="hidden" name="nak_id" id="nak_id" value="${item.nak_id }"/>
				...
				<div class="t_list_area">
					<div class="listtop floats">
						...
						<div class="listnum" align="right">
							...
							<br>
							<a href="/info/admin/riverfishing.do" class="btn_size1 btn_violet">최신순</a>
							<a href="/info/admin/riverlatest.do" class="btn_size1 btn_violet">등록순</a>
							<a href="/info/admin/riverKor.do" class="btn_size1 btn_violet">한글순</a>
						</div>
					</div>
					<table class="t_list">
						<colgroup>
				...
				<div align="right"><a href="#;" onclick="fncGroupListDelete()" class="btn_size1 btn_violet">삭제</a><a href="/info/admin/write_river.do" class="btn_size1 btn_violet">글쓰기</a></div>
				<!-- 페이지 { -->
				<div id="pagenation">
...
function fncGroupListDelete() {
	if(fncManageChecked()) {
	    if(confirm("삭제하시겠습니까?")) {
            document.listform.action = "/info/admin/gotrash_riverlist.do";
