...
<div class="page bg-white">
	<div class="page-content p-10">
		...
		<form:form commandName="naksinuriZisikinVO" id="ctiListFormZazu${CUSTOM_UNIQ_KEY}" name="ctiListFormZazu${CUSTOM_UNIQ_KEY}" method="post">
		<div class="panel">
			...
			<div class="panel-title pl-0 pr-0 pt-5 pb-10">
				...
				<div class="float-right">
					<ul class="pagination pagination-sm pagination-gap">									
						<ui:pagination paginationInfo = "${paginationInfo}" type="admin_top" jsFunction="fn_egov_link_page_zazu${CUSTOM_UNIQ_KEY}" />
		           	...
		           	<c:forEach var="item" varStatus="status" items="${list}">
			           	<tbody class="table-section" data-plugin="tableSection">
							<tr>
								...
								<td class="text-middle text-center">${item.zazu_ques}</td>
							</tr>
						</tbody>
						<tbody>
						 	<tr>
								<td class="text-middle text-center" colspan="2">${item.zazu_answ}</td>
							</tr>
						</tbody>
					</c:forEach>
					<%-- <tfoot>
...
fn_script += '	function fn_egov_link_page_zazu${CUSTOM_UNIQ_KEY}(pageNo){													';
