...
function fnSelectInfs(pageIndex) {
	...
	$("#frm").attr("action", "${pageContext.request.contextPath}/admin/info/riverfishing/trash.do");
	$("#frm").submit();
...
<input type="hidden" name="fishing_type" id="fishing_type" value="riverfishing" />
...
<%@include file="admin_leftTab.jsp"%>
	<div id="container">
		...
		<div id="content">			
			...
			<h1 class="bigTit"><i class="fa fa-trash" aria-hidden="true"></i> 민물낚시 휴지통 <div class="trash_box"><%@include file="trash_selectBox.jsp" %></div></h1>
...
<!-- 						<li><a href="#;">상세내용</a></li> -->
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="5">
									...
									<button class="btn_size1 btn_violet" onclick="fnSelectInfs(1);">검색</button>
								</td>
							</tr>
						</tbody>
					</table>
				...
				<!-- } 검색 영역 -->
	<form:form commandName="riverform" id="riverform" name="listform" method="post" enctype="multipart/form-data" >
		...
		<input type="hidden" name="nak_id" id="nak_id" value="${item.nak_id }"/>
				...
				<div class="t_list_area">
					<div class="listtop floats">
						<div class="totalnum">총 <b>${admin_total}</b>건 <br><span>전체선택</span><input type="checkbox" name="checkAll" class="check2" onclick="javascript:fncCheckAll()" title="전체선택"></div>
						<div class="listnum" align="right">
							...
							<br>
							<a href="/admin/info/riverfishing/trash.do" class="btn_size1 btn_violet">최신순</a>
							<a href="/admin/info/riverlatest/trash.do" class="btn_size1 btn_violet">등록순</a>
							<a href="/admin/info/riverKor/trash.do" class="btn_size1 btn_violet">한글순</a>
						</div>
					</div>
					<table class="t_list">
						<colgroup>
...
function view2(nak_id){
	var form = document.getElementById('riverform');
	$('#nak_id').val(nak_id);
...
function fncGroupListDelete() {
	if(fncManageChecked()) {
	    if(confirm("삭제하시겠습니까?")) {
            document.listform.action = "/info/admin/deleter.do";
...
function fncGroupListRestore() {
	if(fncManageChecked()) {
	    if(confirm("복구하시겠습니까?")) {
            document.listform.action = "/info/admin/restorer.do";
            document.listform.submit();
	    }
	}
}
