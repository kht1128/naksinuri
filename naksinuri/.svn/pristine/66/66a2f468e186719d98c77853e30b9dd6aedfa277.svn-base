...
<!-- 				<li class="on"><a href="/share/zisik/list.do">누리지식인</a></li> -->
	...
	<form id="frm" name="frm" method="post">
		...
		<section class="list_box">
			<div class="list_searchbox">
				...
				<button class="searchBtn nomargin"><span class="blind">검색</span><i class="fa fa-search" aria-hidden="true"></i></button>
			</div>
					...
					<tbody>
						<c:forEach var="item" items="${zisik_list }">
							<tr>
								...
								<td class="subject align_left"><a href="#"onclick="view2('${item.nuri_q_num}')">${item.nuri_q_subject }</a></td>
	...
	<script type="text/javascript">
		...
		function fnSelectInfs(pageIndex) {
			...
			$("#frm").attr("action", "${pageContext.request.contextPath}/share/zisik/list.do");
			$("#frm").submit();
