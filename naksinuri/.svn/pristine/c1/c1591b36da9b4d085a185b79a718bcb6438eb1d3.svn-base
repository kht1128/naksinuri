...
<form:form commandName="CodeSetVO" id="modalMemberWriteFormSido"
	name="modalMemberWriteFormSido" method="post">
	<input type="hidden" name="CD_MASTER_ID" value="" />
</form:form>

<form:form commandName="eduCenterMainVO" id="dataCRSForm"
	name="dataCRSForm" method="post">
	<input type="hidden" name="CRS_SN" value="" />
	<input type="hidden" name="IS_TABLE_TR" value="Y" />
</form:form>

<form:form commandName="eduCenterMainVO" id="searchCRSForm"
	name="searchCRSForm" method="post">
...
<form:form commandName="myHistoryVO" id="listForm" name="listForm"
	method="post">
	...
	<div class="content respon3">
		<section id="writeForm" class="write_box">
			<h2>교육과정선택</h2>
			<p class="dottedbox">
				<b>수강신청문의</b> (전화) 1833-7139 | (팩스) 0505-742-1004 | (이메일) naksinuri@fipa.or.kr
			</p>
			<div class="list_searchbox mt-30">
				<span class="ml-5 mr-5 basic_input text-middle">교육일정</span> <input
					type="text" id="CRS_STR_DT"
					class="frm_input basic_input wp15 text-center"
					value="${CRS_STR_DT}" autocomplete="off" readonly /> <span
					class="ml-5 mr-5 basic_input text-middle">부터 ~</span> <input
					type="text" id="CRS_END_DT"
					class="frm_input basic_input wp15 text-center"
					value="${CRS_END_DT}" autocomplete="off" readonly /> <span
					class="ml-5 mr-5 basic_input text-middle">까지</span> <select
					class="basic_select wp30" title="검색조건선택" id="CRS_GRP_CD" />
				<option value="${item.CD_ID}">지역조건 전체보기</option>
				<c:forEach var="item" items="${list_edu_grp_cd}">
					<option value="${item.CD_ID}"
						<c:if test="${CRS_GRP_CD eq item.CD_ID}">selected</c:if>>${item.CD_NM}</option>
				</c:forEach>
				</select>
				<button class="searchBtn btn_search mr-0" id="searchBtn">
					<span class="blind">검색</span><i class="fa fa-search"
						aria-hidden="true"></i>
				</button>
			</div>
			<div class="list_searchbox mt-30 mb-5">
				<select class="basic_select wp100" title="교육과정선택"
					style="border: 2px solid #000;" name="RMNDR_CRS_SN"
					onchange="sel_crs_data(this)" id="target_sel_crs_data"
					data-fail-message="신청하고자 하는 교육과정을 선택해주세요." required />
		...
		<section id="writeForm" class="write_box mt-30">
			<h2>수강신청서</h2>
			<p class="dottedbox red-600 font-weight-bold mb-30">이름,생년월일,연락처는
				로그인(본인인증)시 본인여부 확인에 사용되므로 불일치시 로그인이 불가능 할 수 있으므로 주의하여 입력해주시기 바랍니다.</p>
			<div class="agree_box">
				<h3 class="">기본회원정보</h3>
				<table class="basic_tbl mt-10">
					...
					<tbody>
						<tr>
							...
							<td><input type="text" class="basic_input w100 readonly"
								placeholder="성함을 입력해주세요." name="RMNDR_MBR_NM_CHK"
								value="${MBR_NM }" data-fail-message="성함은 필수 입력정보입니다."
								autocomplete="off" required disabled title="성함"> <input
								type="hidden" name="RMNDR_MBR_NM" value="${MBR_NM }" readonly />
							</td>
							<th class="text-center">생년월일 <span class="red-600">*</span></th>
							<td><input type="text"
								class="basic_input w100 mbr-birth-input-pattern readonly"
								name="RMNDR_MBR_BIRTH_CHK" value="${MBR_BIRTH }"
								placeholder="예)19190301"
								data-fail-message="생년월일은 필수 입력정보입니다.<br>19190301 형식으로 8자리를 입력해주세요."
								autocomplete="off" required disabled title="생년월일 예)19190301">
								<input type="hidden" name="RMNDR_MBR_BIRTH"
								value="${MBR_BIRTH }" readonly /></td>
						</tr>
						<tr>
							<th class="text-center">주 소 <span class="red-600">*</span></th>
							<td colspan="3"><input type="text"
								class="basic_input w100px mr-5" id="RMNDR_MBR_ZIP"
								name="RMNDR_MBR_ZIP" placeholder="우편번호" readonly />
								<button type="button"
									onclick="zipcode('RMNDR_MBR_ZIP','RMNDR_MBR_ADDR1','RMNDR_MBR_ADDR2');"
									class="btn">우편번호 찾기</button> <input type="text"
								class="basic_input w100 mt-5" id="RMNDR_MBR_ADDR1"
								name="RMNDR_MBR_ADDR1" value="" placeholder="주소를 입력해주세요."
								data-fail-message="주소는 필수 입력정보입니다." autocomplete="off" required
								readonly title="주소"> <input type="text"
								class="basic_input w100 mt-5" id="RMNDR_MBR_ADDR2"
								name="RMNDR_MBR_ADDR2" value="" placeholder="상세주소를 입력해주세요."
								data-fail-message="상세주소는 필수 입력정보입니다." autocomplete="off"
								title="상세주소">
								<div id="zipcode_layer"
									style="display: none; border: 1px solid #005287; width: 100%; height: 300px; margin: 5px 0; position: relative">
									<img
						...
						<tr>
							...
							<td colspan="3"><input type="text"
								class="basic_input w100 mbr-hp-input-pattern readonly"
								name="RMNDR_MBR_HP_CHK" value="${MBR_HP }"
								placeholder="예)01012345678" data-fail-message="연락처는 필수 입력정보입니다."
								autocomplete="off" required disabled title="휴대전화 (-없이 입력)">
								<input type="hidden" name="RMNDR_MBR_HP" value="${MBR_HP }"
								readonly /></td>
						</tr>
						<tr>
							<th class="text-center">연 락 처(일반전화)</th>
							<td colspan="3"><input type="text"
								class="basic_input w100 mbr-tel-input-pattern"
								name="RMNDR_MBR_TEL" value="" placeholder="예)0260980812"
								data-fail-message="일반전화번호를 입력하세요." autocomplete="off"
								title="일반전화(-없이입력)"></td>
						</tr>
					</tbody>
				</table>
				...
				<table class="basic_tbl mt-10">
					...
					<tbody>
						<tr>
							...
							<td>
								<select class="basic_input" title="수강인구분선택" id="RMNDR_DTL_LICENSE_CD" name="RMNDR_DTL_LICENSE_CD" data-fail-message="수강인구분은 필수 선택정보입니다." <%-- onchange="chageSelect();" --%> required>
								...
								<select class="basic_input" title="해기사면허유무선택" name="RMNDR_SHIP_LICENSE" data-fail-message="해기사면허 선택은 필수 선택정보입니다." required>
									<option value="">해기사면허</option>
									<option value="유">유</option>
									<option value="무">무</option>
								</select> 
								<br>
								<span class="red-600 font-weight-bold">선주 겸 선장은 개인업자로 선택해주시기 바랍니다.</span>
							</td>
						</tr>
						...
						<tr class="<c:if test="${not empty INDVDL_INFO_ST and INDVDL_INFO_ST eq 'N' }">hide</c:if>">
							<th>낚시어선명칭</th>
							<td>
								<input type="text" class="basic_input w100 " name="RMNDR_DTL_NM" value="" placeholder="낚시어선 명칭을 입력하세요." data-fail-message="" autocomplete="off" title="낚시어선명칭">
							</td>
						</tr>
						<tr class="<c:if test="${not empty INDVDL_INFO_ST and INDVDL_INFO_ST eq 'N' }">hide</c:if>">
							<th>신고확인증번호</th>
						...
						<tr class="<c:if test="${not empty INDVDL_INFO_ST and INDVDL_INFO_ST eq 'N' }">hide</c:if>">
							<th>어선번호</th>
							<td>
								<input type="text" class="basic_input w100 ship-cd-input-pattern" name="RMNDR_SHIP_CD" value="" placeholder="어선번호를 입력하세요." data-fail-message="" autocomplete="off" title="어선번호">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<select class="basic_input sel_area_sido wp30" title="낚시어선 신고지 시도선택옵션" name="RMNDR_SIDO_CD" data-fail-message="낚시어선 신고지(시도)는 필수 선택정보입니다." required>
									<option value="">시도 선택</option>
									<c:forEach var="item" items="${list_address_cd}">
										<option value="${item.CD_ID}">${item.CD_NM}</option>
									</c:forEach>
								</select> 
								<select class="basic_input sel_area_signgu wp30" title="낚시어선 신고지 시군구선택선택옵션" name="RMNDR_SIGNGU_CD" data-fail-message="낚시어선 신고지(시군구)는 필수 선택정보입니다." required>
									<option value="">시군구선택</option>
								</select> 
								<input type="text" class="basic_input wp30" name="RMNDR_YMD_NM" value="" placeholder="읍면동을 입력하세요." data-fail-message="" autocomplete="off" title="낚시어선 신고지 읍명동">
							</td>
						</tr>
					</tbody>
				</table>
				...
				<div class="hide" id="smsCheck">
					...
					<div class="mt-10 bg-emphasis">
						<p class="font-weight-bold emphasis">
							귀하는 「신규·재개자 전문교육」 대상이 아닙니다. 기존 낚시어선업 신고자(기간만료 후 재신고자 포함)는 「낚시어선
							전문교육」을 수강하여 주시기 바랍니다.<br> 20.2.21. 이후 최초 신고(신규) 및 영업정지명령을 받은
							후 재개하려는 낚시어선업자(재개자)의 경우
						</p>
						<span class="font-weight-bold emphasis2">온라인교육 수강 여부와 관계없이
							「신규·재개자 전문교육(현장교육, 연간 21시간 이상)」을 수강하여야 합니다.</span>
						<p class="font-weight-bold light-red mt-10">* 기존 낚시어선업
							신고자(기간만료 후 재신고자 포함)는 기존 선택 및「낚시어선 전문교육」을 신청하여 주시기 바랍니다.</p>
					</div>
				</div>
				<%--
			...
			<div id="text_submit">
				<p class="dottedbox red-600  mt-30 mb-5 text-center">
					신청서 제출 시 관리자 검토 후 승인이 완료되면 <b>문자로 안내됩니다.</b>
				</p>
				<p class="dottedbox mt-5 mb-5 text-center">
					(신청서는 순차적으로 검토되어 승인 완료까지 다소 시간이 걸릴 수 있으니 양해 부탁드리며,<br>신청 후 익일
					정오 12시까지 안내 문자 미수신시 1833-7139로 신청 확인 부탁드립니다.)
				</p>
				...
				<p class="dottedbox mt-20  text-center">『낚시전문교육 및 교육기관 지정에 관한
					고시』 제8조에 따라 위와 같이 신청합니다.</p>
			</div>
			<div id="btnArea">
...
$("#btn_submit").click(function() {
	...
	if(form.RMNDR_MBR_NM_CHK.value == '') {
		...
		$(form.RMNDR_MBR_NM_CHK).focus();//모달메세지시 포커스가 빠지므로 의미없음..
		return;
	}
