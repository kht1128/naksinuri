	...
	<div class="content respon3">
		<section id="writeForm" class="write_box">
			...
			<div class="list_searchbox mt-30">
				...
				<label for="CRS_STR_DT" class="hidden">교육일정시작일</label> 
				<input type="text" id="CRS_STR_DT" class="frm_input basic_input wp15 text-center" value="${CRS_STR_DT}" autocomplete="off" placeholder="YYYY-MM-DD" title="교육일정 시작일 예)YYYY-MM-DD"/>
				<span class="ml-5 mr-5 basic_input text-middle">부터 ~</span> 
				<label for="CRS_END_DT" class="hidden">교육일정종료일</label> 
				<input type="text" id="CRS_END_DT" class="frm_input basic_input wp15 text-center" value="${CRS_END_DT}" autocomplete="off" placeholder="YYYY-MM-DD" title="교육일정 종료일 예)YYYY-MM-DD"/>
				<span class="ml-5 mr-5 basic_input text-middle">까지</span> 
				<select class="basic_select wp30" title="지역검색조건선택" id="CRS_GRP_CD"/>
				...
				<button class="searchBtn btn_search mr-0" id="searchBtn"><span class="blind">검색</span><i class="fa fa-search" aria-hidden="true"></i></button>				
			</div>
			<div class="list_searchbox mt-30 mb-5">
				<select class="basic_select wp100" title="교육과정선택" name="RMNDR_CRS_SN" onchange="sel_crs_data(this)" id="target_sel_crs_data" data-fail-message="신청하고자 하는 교육과정을 선택해주세요." required/>
					<option value="">교육과정선택하기 (위에 조회 버튼으로 교육과정을 검색해주세요)</option>
				</select>
			</div>
			<div class="agree_box">
				<table class="basic_tbl mt-10">
					<caption>교육정보(교육지역,교육대상,교육과정명,신청기간,교육일시,안내사항)</caption>
					<colgroup>
						<col width="140" />
						<col />
					</colgroup>
				...
				<table class="basic_tbl mt-10">
					...
					<tbody>		
						<tr>
							...
							<td>
								<input type="text" class="basic_input w100 readonly" value="${MBR_NM }" name="RMNDR_MBR_NM_CHK" placeholder="성함을 입력해주세요." data-fail-message="성함은 필수 입력정보입니다." autocomplete="off" required disabled title="성함">
								<input type="hidden" name="RMNDR_MBR_NM" value="${MBR_NM }" readonly/>
							</td>
							<th class="text-center">생년월일 <span class="red-600">*</span></th>
							<td>
								<label for="RMNDR_MBR_BIRTH_CHK" class="hidden">생년월일</label> 
								<input type="text" id="RMNDR_MBR_BIRTH_CHK" class="basic_input w100 mbr-birth-input-pattern readonly" name="RMNDR_MBR_BIRTH_CHK" value="${MBR_BIRTH }" placeholder="예)19190301" data-fail-message="생년월일은 필수 입력정보입니다.<br>19190301 형식으로 8자리를 입력해주세요." autocomplete="off" required disabled title="생년월일 예)19190301">
						...
						<tr>
							...
							<td colspan="3">
								<label for="RMNDR_MBR_HP_CHK" class="hidden">연락처(휴대폰)</label> 
								<input type="text" id="RMNDR_MBR_HP_CHK" class="basic_input w100 mbr-hp-input-pattern readonly" name="RMNDR_MBR_HP_CHK" value="${MBR_HP }" placeholder="예)01012345678" data-fail-message="연락처는 필수 입력정보입니다." autocomplete="off" required disabled title="휴대전화 (-없이 입력)">
						...
						<tr>
							...
							<td colspan="3">
								<label for="RMNDR_MBR_TEL" class="hidden">연 락 처(일반전화)</label> 
								<input type="text" id="RMNDR_MBR_TEL" class="basic_input w100 mbr-tel-input-pattern" name="RMNDR_MBR_TEL" value="" placeholder="예)0260980812" data-fail-message="일반전화번호를 입력하세요." autocomplete="off" title="일반전화(-없이입력)">
							</td>
						</tr>						
					</tbody>
				</table>
				...
				<table class="basic_tbl mt-10">
					...
					<tbody>
						<tr>
							...
							<td>
								<select class="basic_input" title="수강인구분선택옵션" id="RMNDR_DTL_LICENSE_CD" name="RMNDR_DTL_LICENSE_CD" data-fail-message="수강인구분은 필수 선택정보입니다." required title="수강인구분선택">  
						...
						<tr class="<c:if test="${not empty INDVDL_INFO_ST and INDVDL_INFO_ST eq 'N' }">hide</c:if>">
							<th>낚시터명칭</th>
							<td><input type="text" class="basic_input w100 " name="RMNDR_DTL_NM" value="" placeholder="낚시터 명칭을 입력하세요." data-fail-message="" autocomplete="off" title="낚시터 명칭"></td>
						</tr>
						<tr class="<c:if test="${not empty INDVDL_INFO_ST and INDVDL_INFO_ST eq 'N' }">hide</c:if>">
							<th>허가(등록)번호 </th>
							<td>
								<input type="text" class="basic_input wp100 " name="RMNDR_REG_NUM_CD" value="" placeholder="허가(등록)번호를 입력하세요." data-fail-message="" autocomplete="off" title="허가(등록)번호">
								<%--
								(&nbsp;유효기간&nbsp;:&nbsp;
						...
						<tr>
							<th>허가(등록)신고지 <span class="red-600">*</span></th>
							<td>
								<select class="basic_input sel_area_sido wp30" title="허가등록신고지 시도 옵션선택" name="RMNDR_SIDO_CD" data-fail-message="허가(등록)신고지(시도)는 필수 선택정보입니다." required>  
									<option value="">시도 선택</option> 
									<c:forEach var="item" items="${list_address_cd}">
										<option value="${item.CD_ID}">${item.CD_NM}</option>
									</c:forEach>
								</select>
								<select class="basic_input sel_area_signgu wp30" name="RMNDR_SIGNGU_CD" data-fail-message="허가(등록)신고지(시군구)는 필수 선택정보입니다." title="허가등록신고지 시군구 옵션선택 " required>   
									<option value="" >시군구선택</option>
								</select>
								<input type="text" class="basic_input wp30" name="RMNDR_YMD_NM" value="" placeholder="읍면동을 입력하세요." data-fail-message="" autocomplete="off" title="허가등록신고지 읍면동">
							</td>
						</tr>
					</tbody>
				</table>
...
$("#RMNDR_DTL_LICENSE_CD").on("change", function(){
	...
	var html = '';
	if(text == '법인업자'){
		html += '<span class="ml-20 add_html"><b>*교육책임자 지정 확인서 제출 필수(대표자 본인이 수강 시에도 제출)</b></span>';
...
$("#btn_submit").click(function() {
	...
	var RMNDR_MBR_NM_CHK = form.RMNDR_MBR_NM_CHK.value;
	var RMNDR_MBR_ADDR1 = form.RMNDR_MBR_ADDR1.value;
	var RMNDR_MBR_ADDR2 = form.RMNDR_MBR_ADDR2.value;
	
	alert(RMNDR_MBR_NM_CHK);
	
	
	alert(RMNDR_MBR_ADDR1);
	
	alert(RMNDR_MBR_ADDR2);
