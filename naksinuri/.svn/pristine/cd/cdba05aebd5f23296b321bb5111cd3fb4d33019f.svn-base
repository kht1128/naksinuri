	...
	<div id="container">
		...
		<div id="content">			
			...
			<section id="table-list">
				...
				<div id="tabarea">
					<ul class="floats">
<%-- 						<li><a href="${pageContext.request.contextPath}/admin/sosig/event/list.do" class="on">목록조회</a></li> --%>
...
<!-- 						<li><a href="#;">상세내용</a></li> -->
							...
							<tr>
								...
								<td colspan="5">
									<select class="frm_select" style="width:100px" name="searchType">
										...
										<option value="voc_subject">제목</option>
										<option value="voc_name">작성자</option>
										<option value="voc_type">구분</option>
	...
	<form:form commandName="listform" id="listform" name="listform" method="post" enctype="multipart/form-data" >
		...
		<input type="hidden" name="voc_no" id="voc_no" value="${item.voc_no }"/>
				<!-- 리스트 영역 { -->
					...
					<table class="t_list">
						...
						<thead>
							<tr>
								...
								<th scope="col">답변여뷰</th>														
								<th scope="col">등록날짜</th>														
							</tr>
						</thead>
						<tbody>
						<c:forEach var="item" items="${voc_list}">
							<tr>
								<td class="textcenter"><input type="checkbox" name="delYn" class="check2" value="${item.voc_no}"></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.voc_no}')">${select_total-item.rn+1}</a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.voc_no}')">${item.voc_type}</a></td>
								<td class="textcenter"><a href="#;" onclick="view2('${item.voc_no}')">${item.voc_name}</a></td>
								<td><a href="#;" onclick="view2('${item.voc_no}')">${item.voc_subject}</a></td>
								<td class="textcenter">
									<a href="#;" onclick="view2('${item.voc_no}')">
										<c:choose>
											<c:when test="${item.voc_issecret eq '0'}">
												<span class="label label-outline label-warning label-lg">비공개</span>
											</c:when>
											<c:otherwise>
												<span class="label label-outline label-info label-lg">공개</span>
											</c:otherwise>
										</c:choose>
									</a>
								</td>
								...
								<td class="textcenter">
									<a href="#;" onclick="view2('${item.voc_no}')">
										<fmt:parseDate value="${item.voc_insert_dt}" var="f_data" pattern="yyyy-MM-dd HH:mm:ss.SSS" scope="page"/>
				...
				<div align="right"><a href="#;" onclick="fncGroupListDelete()" class="btn_size1 btn_violet">삭제</a></div>
				<!-- 페이지 { -->
				<div id="pagenation">
...
<script>
function view2(voc_no){
	var form = document.getElementById('listform');
	$('#voc_no').val(voc_no);
	
	form.action="./voc_findCorp.do";
	form.submit();
};
