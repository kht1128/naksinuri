			...
			<div class="form-group row">
				<div class="col-md-12">
					...
					<c:forEach var="item" items="${list_mbr}">
						<span class="badge badge-default mr-5 text-typing-label-byte">${item.MBR_ID}(${item.MBR_NM})</span>
					</c:forEach>
				</div>
			</div>
			<div class="float-right">
				<button type="button" class="btn btn-primary btn-outline btn-act-send-kakao">보내기</button>
