...
<div id="container">
		...
		$(document).ready(function(){
			...
			$("#opener_save").click(function(){
				if(confirm('위 정보로 데이터를 저장하시겠습니까?')) {
					...
					location.href="/info/admin/indPreviewUpdate.do?san_sn="+san_sn+"&pid="+preg_pid;
				}
			});
		});
	</script>
	...
	<input type="hidden" name="san_sn" id="san_sn" value="${preview.san_sn}">
	<input type="hidden" name="preg_pid" id="preg_pid" value="${preview.preg_pid}">
		...
		<div class="leftconBox">
			<section id="basicInfo">
				<div class="business_logo">
					<c:choose>
						<c:when test="${pmimg.orignl_file_nm ne null}">
							<img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${preview.san_img}"/>&fileSn=<c:out value="${pmimg.file_sn}"/>' alt=<c:out value="${pmimg.orignl_file_nm}"/> />
						</c:when>
						<c:when test="${porgmimg.orignl_file_nm ne null}">
							<img src='<c:url value='/naksinuri_original/cmm/fms/getImage.do'/>?atchFileId=<c:out value="${info.san_img}"/>&fileSn=<c:out value="${porgmimg.file_sn}"/>' alt=<c:out value="${porgmimg.orignl_file_nm}"/> />
						</c:when>
						<c:otherwise>
...
function delpreview() {
	if(confirm('등록된 미리보기를 정말 삭제하시겠습니까?\n\n삭제한 데이터는 복구가 불가능합니다.')) {
		var san_sn = $('#san_sn').val();
		location.href = "/info/admin/indPreviewdel.do?san_sn="+san_sn;
	}
}
