...
<input type="hidden" name="MHC_SN" value=""/>
<input type="hidden" name="MBR_SCRTY_KEY" value=""/>
</form:form>

<form:form commandName="eduAdmMbrHpChngVO" id="listForm" name="listForm" method="post">
    <!-- Page -->
    <div class="page">
            ...
            <div class="panel">
              <div class="panel-body">
					...
					<!-- table:start -->
	          		...
	          		<thead>
	          			<tr>
	          				<th class="text-middle text-center border-r" colspan="4">교육 이수자</th>
	          				<th class="text-middle text-center border-r" colspan="3">핸드폰 명의자</th>
	          				<th class="text-middle text-center border-r" rowspan="2">신청일자<br>(처리일자)</th>
	          				<th class="text-middle text-center" rowspan="2">비고</th>
	          			</tr>
	          			<tr>
							<!-- <th class="text-center hide-cell-exceldown">
							...
							<th class="text-middle text-center border-r">어선명</th>
							<th class="text-middle text-center">성명</th>
							<th class="text-middle text-center">생년월일</th>
							<th class="text-middle text-center">휴대전화번호</th>
							
							<!-- <th class="text-center hide-cell-exceldown">관리</th>	 -->
	                	</tr>       				
	             	</thead>
					<tbody>
						<c:if test="${empty list}">
							<tr><td colspan="8" class="text-middle text-center table-active">본인명의 휴대폰 미소지자 정보 변경을 신청한 인원이 없습니다.</td></tr>	  
		            	</c:if>
		            	<c:forEach var="item" varStatus="status" items="${list}">
		            				            	
		            		<tr class="<c:if test="${status.index%2 eq 0}">table-active</c:if>">
		            			<%-- <td class="text-center hide-cell-exceldown">
			            			<span class="checkbox-custom checkbox-primary">
                            	...
                            	<td class="text-middle text-center">${item.MHC_EDU_NM }</td>
                            	<td class="text-middle text-center">${item.MHC_EDU_BRTHDY }</td>
                            	<td class="text-middle text-center">${item.MHC_EDU_HP }</td>
                            	<td class="text-middle text-center">${item.MHC_EDU_SHIP}</td>
                            	<td class="text-middle text-center">${item.MHC_NM }</td>
                            	<td class="text-middle text-center">${item.MHC_BRTHDY }</td>
                            	<td class="text-middle text-center">${item.MHC_HP }</td>
                            	<td class="text-middle text-center">
                            		<fmt:parseDate var="parseregdatestring" value="${item.MHC_REG_DT }" pattern="yyyy-MM-dd HH:mm:ss" />
         							<fmt:formatDate var="reg_dt" value="${parseregdatestring}" pattern="yyyy.MM.dd HH:mm" />
         							<c:out value="${reg_dt }"/>
         							<c:if test="${not empty item.MHC_UPD_DT }">
         								<fmt:parseDate var="parseupddatestring" value="${item.MHC_UPD_DT}" pattern="yyyy-MM-dd HH:mm:ss" />
         								<fmt:formatDate var="upd_dt" value="${parseupddatestring}" pattern="yyyy.MM.dd HH:mm" />
         								<br><span class="red-600">(<c:out value="${upd_dt}"/>)</span>
         							</c:if>
                            	</td>
                            	<td class="text-middle text-center">
	                            	<c:if test="${not empty item.MHC_MBR_ID }">
	                            		<a href="#;" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row clk_mod_data" data-toggle="tooltip" data-original-title="수정" 
						                	data-mbr-id="${item.MHC_MBR_ID }" data-mhc-sn="${item.MHC_SN }" 
  ...
  //회원상세정보보기(자세히보기)
   ...
   $(".clk_mod_data").click(function() {
   	...
   	form.DEL_NOW.value = '';
   	form.MHC_SN.value = $(this).attr('data-mhc-sn');
   	form.action = '';
